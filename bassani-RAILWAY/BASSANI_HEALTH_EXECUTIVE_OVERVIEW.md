...
- **Welcome Pack as four managed documents** — the Welcome Pack (Help Me Budget, Welcome Letter, Price List, Product Brochure) is managed as four independent documents, each with its own version history. If the price list changes, upload only the new price list; the brochure and other files are untouched. When a welcome pack email is sent, the active version of each document is attached automatically. Previous versions of each document are permanently archived and restorable individually.
...
| Reseller quote flow — resellers build orders via cart and can either place them (confirmed immediately, with the portal's own stock and credit checks standing in for manual staff review) or save them as a draft first if they're not ready to commit; drafts are managed in My Quotes with edit/send/confirm available whenever needed, and remain visible to Bassani staff to assign, track, or confirm on the reseller's behalf | Live |
...
| Operations Monitor (Phase 23) — a live, read-only big-screen display that shows the full order pipeline in real time. Colour-coded cards flag orders approaching the 72-hour fulfilment deadline. No login required — a token-protected URL is opened on any TV or display. The monitor gives the team at-a-glance visibility into what is on track, what needs attention, and what is overdue, without opening the ticket list. The KPI strip shows Overdue, At Risk, Compliance Hold (orders in QA or RP review), Completed Today, and a per-stage breakdown with the age of the oldest live order. No financial data is displayed — the monitor focuses on pipeline throughput and bottlenecks. | Live |
| Onboarding Monitor (Phase 23.2) — a second big-screen display, same idea applied to the customer onboarding pipeline instead of order fulfilment: every application in flight, from submission through to a fully approved and welcomed customer, colour-coded by how long it has been sitting at its current step. Its own independent token-protected URL, managed separately from the order monitor's. A customer only counts as fully onboarded once their profile has actually been created **and** the welcome pack has actually been sent — an approval that succeeded but whose welcome pack failed to send shows up as its own flagged state rather than quietly looking finished. Requested directly by the executive team after seeing the order pipeline monitor in use, as the first of a planned family of similar dashboards for other parts of the business. | Live |
| Manufacturing Monitor (Phase 23.3) — a third big-screen display for the GACP manufacturing facility, showing every in-progress Manufacturing Order tied to a customer order: what needs to be produced, how much is left, and which order and customer it's for, so the floor can prioritise its own queue at a glance instead of waiting for the once-a-day email. Its own independent token-protected URL. Also brought a smaller, related improvement to the original order-pipeline monitor: any order with a line still waiting on stock or production now carries a visible "Backorder" flag directly on the board, so a stalled order is never mistaken for one that's simply progressing normally. A same-day follow-on added a logged-in, searchable **Manufacturing Orders** page for staff (distinct from the existing Backorders page, which only shows deliveries already short — a Manufacturing Order can exist well before that point), plus a direct link into it from any affected order's own page. | Live |
| Phase 24: Analytics Export — staff with the export permission can select any SA financial year (current or prior years) and any month within it, or view the full year aggregated, then export all six analytics reports (Turnover, Best Sellers, Best Customers, Best Resellers, Dead Stock, Category Performance) to a single multi-tab Excel file with one click. All reports are scoped to the user's selected warehouse; selecting "All warehouses" returns group-wide data. The file is generated client-side with no server-side dependency. | Live |
| Customer self-service ordering (Phase 25) — customers Bassani chooses to enable can now log into the portal themselves to place orders, track their status, and view invoices, without going through a Sales Agent or calling in. Access is granted per customer by an admin, not automatic, and a business can have more than one person on the account with a login (each sharing the same order and invoice history). No commission is paid on self-service orders directly, but if the customer is already linked to a Sales Agent, that agent continues earning commission exactly as before — this reduces manual order-entry load on staff without changing how any existing agent gets paid. The ordering screen is built mobile-first, since most customers are expected to order from a phone rather than a laptop — filters and the order summary adapt to fit a small screen without losing any functionality. Placing an order now confirms it in one step (with a Save as Draft option for anyone not ready to commit), rather than requiring a second, easy-to-miss manual confirmation afterward — the same change also applies to Sales Agent orders, removing a manual review step that Bassani staff no longer need to perform themselves, since the portal's own stock and credit checks already cover it. A recurring order can now be set up in the same step as placing it, rather than as a separate follow-up action. | Live |
...
| Phase 25 (remainder): WhatsApp ordering bot — lets a customer browse the catalogue, request a quote, and check order status from WhatsApp; still requires the customer to tap a secure link into the portal to actually approve and place an order | Concept stage |
...
- **Batch numbers are generated, never typed.** The operator picks the batch type and product; the system builds the batch number to Bassani's own V6 standard, with the correct sequence and date. This eliminates the formatting inconsistencies present in the current spreadsheets, which are a real audit exposure.
