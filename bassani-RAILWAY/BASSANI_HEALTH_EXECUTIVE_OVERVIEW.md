...
| Onboarding Monitor (Phase 23.2) — a second big-screen display, same idea applied to the customer onboarding pipeline instead of order fulfilment: every application in flight, from submission through to a fully approved and welcomed customer, colour-coded by how long it has been sitting at its current step. Its own independent token-protected URL, managed separately from the order monitor's. A customer only counts as fully onboarded once their profile has actually been created **and** the welcome pack has actually been sent — an approval that succeeded but whose welcome pack failed to send shows up as its own flagged state rather than quietly looking finished. Requested directly by the executive team after seeing the order pipeline monitor in use, as the first of a planned family of similar dashboards for other parts of the business — a Manufacturing Orders / Backorders monitor for the production facility is the next one under consideration. | Live |
