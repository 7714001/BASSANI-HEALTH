...
- **Welcome Pack as a managed bundle** — the Welcome Pack is not a single document but a set of files (typically one Excel price list and several PDFs). The portal manages these as a versioned bundle: all files are uploaded together as one release, and when a welcome pack email is sent, all files in the active bundle are automatically attached. If the price list changes, upload a new bundle and all subsequent welcome pack emails will use the updated files. Previous bundles are archived and restorable.
