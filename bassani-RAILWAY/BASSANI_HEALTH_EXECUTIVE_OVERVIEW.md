...
| Customer self-service ordering (Phase 25) — customers Bassani chooses to enable can now log into the portal themselves to place orders, track their status, and view invoices, without going through a Sales Agent or calling in. Access is granted per customer by an admin, not automatic, and a business can have more than one person on the account with a login (each sharing the same order and invoice history). No commission is paid on self-service orders directly, but if the customer is already linked to a Sales Agent, that agent continues earning commission exactly as before — this reduces manual order-entry load on staff without changing how any existing agent gets paid. | Live |
