...
async def resolve_parent_category_product_ids(odoo, parent_category_id: str) -> list:
    ...
    covered by any active parent category). One Odoo call per resolution,
    batched over the category-id union — never one call per doc."""
    ...
    try:
        oid = ObjectId(parent_category_id)
    except Exception:
        return []
    doc = await col("parent_categories").find_one({"_id": oid, "active": True})
    if not doc:
        return []
    category_ids, handpicked_ids = resolve_membership_ids(
        odoo, doc.get("odoo_category_ids", []), doc.get("product_ids", [])
    )
