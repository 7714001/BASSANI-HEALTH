...
async def get_profile(current_user: dict = Depends(get_current_user)):
    ...
    doc = await col("users").find_one({"_id": current_user["_id"]})
    return {
        "id":            uid,
...
async def update_profile(
    ...
    await col("users").update_one({"_id": current_user["_id"]}, {"$set": updates})

    await audit_log(
        user=current_user,
        action="profile.updated",
        entity_type="user",
        entity_id=str(current_user.get("_id") or current_user.get("id", "")),
...
async def get_signature(current_user: dict = Depends(require_permission("signing_authority.sign"))):
    ...
    uid = str(current_user.get("_id") or current_user.get("id", ""))
    doc = await col("users").find_one({"_id": current_user["_id"]}, {"has_signature": 1})
