...
- `backend/routes/production_routes.py` — Phase 13.0 vault module: product shortcodes (`product_shortcodes`, seeded from `backend/data/product_shortcodes.json`), batch generator + registry (`batch_registry`), vault movements + ledger (`vault_movements`), GACP readiness probe (`GET /odoo-probe`), staged-queue sync (`POST /sync-staged`). Permissions: `production.batch_generate` / `production.vault` / `production.manage`.
