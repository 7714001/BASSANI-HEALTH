...
- `backend/routes/production_routes.py` — Phase 13.0 vault module: product shortcodes (`product_shortcodes`, seeded from `backend/data/product_shortcodes.json`; `import_ref` per product from `import_product_refs.json`), import suppliers (`import_suppliers`, seeded from `import_suppliers.json`), batch generator + registry (`batch_registry`, BH families only via `POST /batches` — **the `import` family is refused there** (422), created solely inside `_create_import_batch`/`receive_import` so an imported batch can never exist without its `s6_receipts` entry), vault movements + ledger (`vault_movements`), S6 receiving (`POST /vault/receive-import` — one action creates BI batch + vault receive + `s6_receipts` register entry; `GET /s6-register`), GACP readiness probe (`GET /odoo-probe`), staged-queue sync (`POST /sync-staged`), test-data purge (`POST /purge-test-data`, super admin, blocked once records are Odoo-synced). S6 receipts are quarantined until RP release (`POST /s6/{id}/release|query`, `production.rp_release`; flag resolution via `/s6/{id}/resolve-flag`, `production.manage`); the quarantine gate in `create_movement` denies by default — an import-family batch with no `s6_receipts` record at all is blocked, not silently allowed. Permissions: `production.batch_generate` / `production.vault` / `production.rp_release` / `production.manage`.
