...
- `frontend/src/components/ProductPickerDrawer.js` — right-side drawer for quote builder (staff only). Categories from `/api/products/categories`, variants derived client-side from `display_name` pattern `"Name (Variant)"`, `category_id` int param to backend. Category + variant filters use `SearchableSelect` (exported — trigger + dropdown panel with search input, outside-click close, auto-focus; reused by BatchRegistry.js's Link Product picker). Also exports `MultiSearchableSelect` (13.0/7.12) — same look, multi-value: panel stays open on pick, per-row checkmarks, trigger shows a "N selected" count; used by ParentCategories.js for picking several Odoo categories into one Parent Category. Reuse this for any future multi-select-from-a-small-list need rather than forking `SearchableSelect` again.
- `backend/parent_categories.py` — shared helpers for the Parent Categories feature (7.12): `resolve_parent_category_product_ids()` (membership = union of `odoo_category_ids` ∪ `product_ids`, including the `"uncategorised"` sentinel), `sync_reseller_catalog_additions()` (idempotently `$addToSet`s hand-picked products into `reseller_catalog.product_ids`). Kept outside both consuming route files to avoid a route-importing-route circular import.
- `backend/routes/parent_category_routes.py` — CRUD for portal-only Parent Categories (7.12), gated by `products.manage`. Never writes to Odoo — contrast with `product_routes.py`'s `/categories` endpoints, which manage real Odoo `product.category` records. `POST /preview` resolves an unsaved category/hand-pick selection into the real product list for the edit-modal live preview, flagging each match's `source` (`category`/`handpick`) and `catalog_visible` status — surfaces the fact that category-matched products are NOT auto-added to `reseller_catalog` the way hand-picks are.
- `frontend/src/views/ParentCategories.js` — `/catalogue/parent-categories` ("Parent Categories" nav item, `products.manage`): admin-only page for defining portal-only product groupings for reseller browsing. Two membership pickers per group: `MultiSearchableSelect` for bulk Odoo categories, and a local `ProductMultiPicker` (debounced live search against `GET /api/products/`) for hand-picking individual variants (e.g. a weekly-rotating "Specials" bucket). A debounced live **Preview** panel in the edit modal calls `POST /api/parent-categories/preview` as selections change, showing the resolved product list before saving with a per-row badge (Visible / Will be added / Hidden — not in catalog) so the admin can catch a category-matched-but-invisible product before saving, not after. Blue "portal-only, never touches Odoo" banner — deliberate visual contrast with `ProductCategories.js`'s red "writes directly to Odoo" banner. Sibling nav item "Categories" was renamed **Odoo Categories** for the same reason.
