...
| 13 | Production and Cultivation Module | In Progress — 13.0 Vault Movement Module (Track A starter) scoped from live Patricia Vault Transaction Logbook: batch ID generator + registry, vault movement form with staged Odoo writes (`GACP_ODOO_WRITES` switch, pending GACP access confirmation), vault ledger, new `vault_custodian` role + `production.*` permissions. Remainder of phase still concept — needs SAHPRA scoping |
