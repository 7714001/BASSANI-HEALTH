...
- `backend/services/vault_odoo.py` — `VaultOdooWriter`: sole Odoo gateway for the vault module. `GACP_ODOO_WRITES=off` (default) stages every op (exact intended payload stored on the movement doc, `odoo_sync: "staged"`); `on` executes live (internal transfers, no-BoM `mrp.production` for the manicuring split). Staged queue is an outbox, not a ledger — must be flushed once live.
