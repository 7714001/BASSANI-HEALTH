...
- `backend/routes/production_routes.py` — Phase 13.0 vault module: product shortcodes (`product_shortcodes`, seeded from `backend/data/product_shortcodes.json`), batch generator + registry (`batch_registry`), vault movements + ledger (`vault_movements`), GACP readiness probe (`GET /odoo-probe`), staged-queue sync (`POST /sync-staged`), test-data purge (`POST /purge-test-data`, super admin, blocked once records are Odoo-synced). Permissions: `production.batch_generate` / `production.vault` / `production.manage`.
...
- `frontend/src/components/ProductionGuide.js` — `ProductionGuideButton`: self-contained "Guide" button + reference modal on both production pages. Plain-language V6 batch anatomy (colour-coded example), family/stage/packaging tables, movement type explanations, Staged-label meaning. Update this if the V6 standard ever gains a prefix family (e.g. pending BIBG clarification).
