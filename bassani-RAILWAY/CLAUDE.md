...
- `backend/routes/parent_category_routes.py` — CRUD for portal-only Parent Categories (7.12), gated by `products.manage`. Never writes to Odoo — contrast with `product_routes.py`'s `/categories` endpoints, which manage real Odoo `product.category` records. Categories can nest one level deep via `parent_id` (e.g. "Flower" containing "Indoor"/"Exotic"/"Greendoor"/"Greenhouse", each wrapping its own real Odoo categories — mirrors how Bassani's actual Odoo categories are structured, family × grade). `_validate_parent_id()` enforces exactly two levels: a category already nested can't be chosen as a parent, and a category with existing children can't itself be nested. `PUT` takes an explicit `clear_parent` bool since `Optional[str]=None` can't distinguish "leave unchanged" from "unparent." Deleting a category with children is blocked (400) until they're reassigned or removed. `POST /preview` resolves an unsaved category/hand-pick selection (this doc's own direct membership only, not descendants) into the real product list for the edit-modal live preview, flagging each match's `source` (`category`/`handpick`) and `catalog_visible` status — surfaces the fact that category-matched products are NOT auto-added to `reseller_catalog` the way hand-picks are. `PUT /category-mapping/{odoo_category_id}` (used by `CategoryMapping.js`) reassigns a single Odoo category as a **move**: pulls it out of every doc that currently has it before adding it to the new target, so the bulk mapping page's one-category-has-one-home model holds regardless of the underlying many-to-many schema.
- `frontend/src/views/ParentCategories.js` — `/catalogue/parent-categories` ("Parent Categories" nav item, `products.manage`): admin-only page for defining portal-only product groupings for reseller browsing. Two membership pickers per group: `MultiSearchableSelect` for bulk Odoo categories, and a local `ProductMultiPicker` (debounced live search against `GET /api/products/`) for hand-picking individual variants (e.g. a weekly-rotating "Specials" bucket). A "Parent category (optional)" `Select` lets a category nest one level under a top-level one (options restricted to top-level-only categories; hidden with an explanatory note if the category being edited already has its own children). A debounced live **Preview** panel in the edit modal calls `POST /api/parent-categories/preview` as selections change, showing the resolved product list before saving with a per-row badge (Visible / Will be added / Hidden — not in catalog); shows an amber note when the category has children, since the preview only resolves this doc's own direct membership, not its descendants'. Blue "portal-only, never touches Odoo" banner — deliberate visual contrast with `ProductCategories.js`'s red "writes directly to Odoo" banner. Sibling nav item "Categories" was renamed **Odoo Categories** for the same reason.
- `frontend/src/views/CategoryMapping.js` — `/catalogue/category-mapping` ("Category Mapping" nav item, `products.manage`): the bulk-setup companion to `ParentCategories.js`. One row per real Odoo category (from `GET /api/products/categories`) with a Parent Category `Select` and a Sub Category `Select` (populated from that parent's children via `parent_id`) — mirrors exactly how Bassani planned their real category mapping on paper, rather than requiring N trips into individual Parent Category edit modals. Each dropdown change saves immediately via `PUT /api/parent-categories/category-mapping/{odoo_category_id}` (no batch "Save" step, same auto-save-per-action convention as the reseller-catalog toggle in `Products.js`). Search box + "Unmapped only" `FilterPill` toggle; subtitle shows a running "N of M mapped" count. Blocked with an `EmptyState` prompting category creation first if no Parent Categories exist yet.
