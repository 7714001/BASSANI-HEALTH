...
- **Self-service registration:** Public route `/apply` — customers register without staff involvement. Reseller referral links use `?ref=RESELLER_CODE`. Customer signs only the Customer Information Form in-browser + uploads CIPC. NDA and Store Onboarding Agreement are sent separately by admin after review. The wizard is 5 steps: (0) Business Details — Business Type dropdown at top (6 sector-based options — Pharmacy, Dispensary, Wellness Centre, Section 22C Facility, Sole Proprietor, Other, Sole Proprietor supported); form adapts instantly: Sole Proprietors see no Company Reg or Trading Name fields, "Registered Company Name" label becomes "Business / Trading Name"; (1) Primary Contact — SA ID required + Luhn-validated, position required, SA phone format enforced; (2) Business Address — suburb, province, and postal code all required, postal code validated as exactly 4 digits; (3) Additional Info; (4) Sign Documents. Same Business Type dropdown + adaptive fields appear at the top of Step 1 in the reseller-initiated `CustomerOnboarding.js` wizard.
