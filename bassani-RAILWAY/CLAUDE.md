...
- `backend/routes/variant_alias_routes.py` — Variant Aliases tab's backend (7.13). Admin-editable `{CODE: "Friendly Name"}` dictionary (`portal_settings._id: "variant_label_aliases"`) mapping cryptic Odoo attribute-value codes (`GD`, `GH`, `EXO`, `IND`, confirmed inconsistent across product lines — Flower and Pre Roll don't share the exact same set) to names resellers actually understand. `GET` open to any authenticated user (resellers read it client-side); `PUT` gated on `products.manage` (same permission as the rest of the Parent Categories page). Applied via `applyVariantAlias()` in `UI.js`, called from `getVariantLabel()` in `ResellerCatalog.js` and the reseller order cart in `Views.js` only — the admin Products view's `getVariantLabel()` call sites pass no alias map (default `{}`) and keep showing Odoo's raw codes.
