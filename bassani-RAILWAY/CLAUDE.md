...
| 24 | Reports Export and Period Selection | Complete — SA financial year selector (current + 2 previous), month pills (Mar–Feb order), Full Year aggregation mode, Excel export button (all 6 reports in one workbook). Gated by `reports.export` permission. `xlsx` (SheetJS) client-side; backend endpoints accept `from_date`/`to_date` range params. |
