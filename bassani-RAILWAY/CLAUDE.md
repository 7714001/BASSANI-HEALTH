...
- `frontend/src/views/DocumentTemplates.js` — version-controlled onboarding template management (upload, activate, rollback, version history). Three templates: NDA, Store Onboarding Agreement, Customer Information Form (TQA removed).
...
- `frontend/src/views/CustomerApplicationDetail.js` — application detail for admin review. "Generate and Send Documents" button triggers signing session creation; signing session status card shows which docs the customer has signed.
...
| 8 | Order Workflow and Ticketing System | In Progress — core pipeline built; partial fulfilment/backorder flow, invoice_policy_block safeguard, per-user document signing, self-service customer registration, product picker drawer all built. 8.24–8.36 complete: invoice lifecycle actions, credit notes, address management, Order Passport, reseller traceability, ticket linking + inbox integration, SO # column on ticket list. 8.37: customer onboarding redesign — TQA removed; customer submits CIF + CIPC only; admin sends NDA + SOA via 30-day signing session (public /sign/:token page); approval gates on all 4 docs. Staff account creation outstanding. |
...
- **Signing session flow:** After admin reviews initial 2 docs, they click "Generate and Send Documents" on the application detail page. System creates a `signing_sessions` MongoDB doc with a 30-day UUID token, sends the customer an email with link `/sign/{token}`. Customer opens the page (no auth), reviews and signs NDA + SOA in-browser using pre-filled data. Signed PDFs are stored in R2 and stamped onto the application. Approval gate requires all 4 docs (CIF + CIPC + NDA + SOA); NDA and SOA must be countersigned before approval.
- **TQA removed:** The TQA document has been removed from all surfaces (templates, doc types, signing flows, document templates admin). The three managed templates are now: Store Onboarding Agreement, Customer Information Form, NDA.
