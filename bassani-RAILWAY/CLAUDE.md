...
- `frontend/src/views/DocumentTemplates.js` — version-controlled onboarding template management. Three single-file PDFs (NDA, Store Onboarding Agreement, Customer Information Form) use `DocTypeCard`. Welcome Pack uses `WelcomePackBundleCard` — multi-file bundle (PDF + Excel), per-file label editing, version history with per-file download drill-down. Switching on `template.is_bundle` from the API drives which card renders.
...
- **TQA removed:** The TQA document has been removed from all surfaces (templates, doc types, signing flows, document templates admin). The three managed templates are now: Store Onboarding Agreement, Customer Information Form, NDA. A fourth managed template — Welcome Pack — is a multi-file bundle (not customer-signed); all files in the active bundle are attached to the welcome pack email.
...
- **Welcome pack:** Admin (`customers.approve_onboarding`) clicks **Send Welcome Pack** after all signing is complete. `POST /{app_id}/send-welcome-pack` fetches all files from the active welcome pack bundle via `get_active_bundle_files("welcome_pack")`, attaches them alongside the countersigned NDA + SOA, and sends `send_customer_welcome_pack()`. Sender's `signing_name`/`signing_title` from their `users` doc appears as the email footer. Stamps `welcome_pack_sent_at`/`welcome_pack_sent_by` on the application.
- **Welcome pack bundle:** The Welcome Pack is a multi-file bundle managed separately from single-file PDFs. Super admin uploads all files together (PDF + Excel accepted) as one versioned bundle via `POST /api/doc-templates/welcome_pack/upload-bundle`. Each file in the bundle has an editable label. Individual files can be downloaded via `GET /api/doc-templates/welcome_pack/bundle/{id}/file/{idx}`. Version history lists bundles with per-file drill-down. The `WelcomePackBundleCard` component in `DocumentTemplates.js` handles the bundle UI; `DocTypeCard` handles the three single-file PDFs unchanged.
