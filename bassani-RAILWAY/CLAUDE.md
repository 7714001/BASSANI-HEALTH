...
**Key files:**
- `backend/server.py` — FastAPI app, startup events, SPA catch-all route. The catch-all uses `os.path.realpath` confinement: any resolved path outside the static build directory silently serves `index.html` (path traversal protection).
...
- `frontend/src/components/ProductPickerDrawer.js` — right-side drawer for quote builder (staff only). Categories from `/api/products/categories`, variants derived client-side from `display_name` pattern `"Name (Variant)"`, `category_id` int param to backend. Category + variant filters use `SearchableSelect` (exported — trigger + dropdown panel with search input, outside-click close, auto-focus; reused by BatchRegistry.js's Link Product picker, and by `ResellerCatalog.js`/`Views.js`'s reseller order cart for their cascading Category → Brand/Grade → Variant filter dropdowns, 7.12 — those two reseller-facing surfaces deliberately use dropdowns, not `ChipRow`/`FilterPill`, since Bassani's real category count doesn't fit a chip carousel; every other filter surface in the app keeps chips). Also exports `MultiSearchableSelect` (13.0/7.12) — same look, multi-value: panel stays open on pick, per-row checkmarks, trigger shows a "N selected" count; used by ParentCategories.js for picking several Odoo categories into one Parent Category. Reuse this for any future multi-select-from-a-small-list need rather than forking `SearchableSelect` again.
...
- `frontend/src/utils/validators.js` — shared field validators: `validateSAID` (13-digit Luhn-checked SA ID number) and `validateSAPhone`. Used by `PublicRegister.js` (customer self-registration) and `Views.js`'s `Resellers()` Sales Agent wizard (Sole Proprietor ID Number field). Never re-implement these elsewhere — import from here.
...
- `frontend/src/views/EmailSettings.js` — Settings > Email Notifications tab. Data-driven, not hand-coded per notification: a `ROUTING_KEYS` metadata array (12 entries, one per `EmailRoutingConfig` field) grouped by a `GROUPS` array (Onboarding & Applications / Orders & Fulfilment / Finance / Production & Vault) into a sidebar-nav + content layout — adding a notification type is one array entry, not a new JSX block. Grouping is a navigation convenience only (no group-level default recipients); `config`/`save()` still read/write the whole `EmailRoutingConfig` object regardless of which group is active. `RoutingSection`/`EmailTagInput` are the reusable per-key card and tag-pill email input. Uses `max-w-5xl` (not the standard admin `max-w-4xl`) since the sidebar needs more width.
