...
- `backend/routes/production_routes.py` — Phase 13.0 vault module: strain shortcodes (`strain_shortcodes`, seeded from `backend/data/strain_shortcodes.json`), batch generator + registry (`batch_registry`), vault movements + ledger (`vault_movements`), GACP readiness probe (`GET /odoo-probe`), staged-queue sync (`POST /sync-staged`). Permissions: `production.batch_generate` / `production.vault` / `production.manage`.
...
- `frontend/src/views/BatchRegistry.js` — `/production/batches`: generator hero (family + strain picker, live ID preview, zero typing), registry table, per-batch timeline modal. Exports `SyncBadge`/`fmtWhen` reused by VaultLogbook.
