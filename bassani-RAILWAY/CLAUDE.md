...
| 8 | Order Workflow and Ticketing System | In Progress — core pipeline built; partial fulfilment/backorder flow, invoice_policy_block safeguard, per-user document signing, self-service customer registration, product picker drawer all built. 8.24–8.34 complete: invoice lifecycle actions (send, PDF, reset-to-draft), invoice type selection (fixed/percentage/delivered), credit notes, customer address management, payment terms, full invoice lifecycle on Invoices page, "Create Ticket" from invoice, Backorders admin view (with SO→passport link), batch/lot numbers on all print documents, MO visibility, Order Passport unified lifecycle view, reseller traceability across all order views. Staff account creation outstanding. |
