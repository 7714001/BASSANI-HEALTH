...
- [ ] **Pagination — not yet built, approach agreed:** flat `skip`/`limit` on the raw rows would split a batch's own stages across pages, which is wrong for a grouped, compliance-relevant view. Batch Registry and Vault Ledger need a **grouped aggregation endpoint** — group by `base_batch_id` in Mongo (not in JS after fetching a flat window), paginate over the *group count*, keep the existing expand-to-fetch-detail behaviour unchanged for what's inside an expanded row. S6 Register isn't grouped — it's a straight `DataTable` + `manualPagination` wire-up, matching `Invoices.js` exactly. Movement History is already flat and ungrouped — same straightforward wire-up.
- [ ] Replace the bespoke `<table>` markup in all three views with the shared `DataTable` component once the above is in place, for visual and interaction consistency with the rest of the app.
