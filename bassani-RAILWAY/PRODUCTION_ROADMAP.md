...
- [x] Parent Categories admin page extended with a "Parent category (optional)" picker (native `Select`, top-level options only) and a "Parent" table column; `PUT` supports an explicit `clear_parent` flag since `Optional[str]=None` can't distinguish "leave unchanged" from "unparent"; deleting a category with existing children is blocked (400) until they're reassigned or removed
...
- [x] **Category Mapping bulk-setup page** (`frontend/src/views/CategoryMapping.js`, `/catalogue/category-mapping`) — one row per real Odoo category with a Parent Category dropdown and a Sub Category dropdown (populated from that parent's children), mirroring exactly how Bassani planned their real mapping on paper. `PUT /api/parent-categories/category-mapping/{odoo_category_id}` performs the assignment as a **move**: pulls the category out of every doc it currently belongs to before adding it to the new target, so the page's one-category-has-one-home mental model holds even though the schema still allows many-to-many (used elsewhere by the hand-pick flow, e.g. Specials). Search box + "Unmapped only" toggle for auditing a large category list in one sitting; subtitle shows a running "N of M mapped" count. Complements rather than replaces the per-category edit modal in `ParentCategories.js`, which remains the tool for naming, hand-picked products, and preview.
