...
- [x] **Bulk Reseller Catalog / MOQ import via the export sheet (added 2026-09-14)** — Bassani asked to bulk-set which products are reseller-catalog-visible and their MOQ from a spreadsheet, rather than toggling one product at a time on the Products page. `GET /api/parent-categories/odoo-export` gained `reseller_visible`/`moq` columns (sourced from the same `reseller_catalog` doc the Products page toggle reads/writes); `frontend/src/components/ResellerCatalogImportModal.js` (new, opened from the Parent Categories toolbar) parses a re-uploaded copy of that sheet client-side, matched on Odoo Product ID only. Two new backend endpoints, sharing one diff function: `POST /api/parent-categories/reseller-catalog-import/preview` (read-only — resolves the uploaded rows against live current state and returns exactly what would change: add/remove/MOQ-changed/unchanged/not-found/duplicate) and `POST /api/parent-categories/reseller-catalog-import/apply` (re-derives the same diff fresh at commit time rather than trusting the preview, and writes only rows that still genuinely differ). The frontend shows the diff as a checkbox-selectable table (default: every changed row checked, unchanged rows collapsed into a count) before any write happens — nothing is applied without an explicit second confirmation, and a partial subset can be applied by unchecking rows. MOQ is forced to 0 (ignored) on any row targeting `reseller_visible=false`, flagged to the user rather than silently dropped, since a lingering MOQ on a hidden product would otherwise resurface with no explanation if it's toggled back on later.
- [x] **Category Mapping "Products" column (added 2026-09-14)** — a sortable count column showing how many products sit directly in each real Odoo category, so staff can immediately tell which unmapped categories are safe to leave alone (0 products) versus which genuinely need mapping. Backed by a new `GET /api/parent-categories/category-product-counts` (one bulk read + tally, deliberately not Odoo's own recursive `product_count` field — see that entry's own note for why). Clicking a non-zero count opens a modal listing the actual products, reusing the existing exact-match `GET /api/products/?category_id=` endpoint so the number and the list can never disagree. An amber banner surfaces the total count of unmapped-but-populated categories with a one-click shortcut to filter+sort straight to them.
...
**Completed:** Sub-deploy 1 (8.1 Roles & Permissions) — 2026-06-19 · Sub-deploy 2 (8.2–8.4 backend) — 2026-06-19 · Sub-deploy 3 (8.5 UI) — 2026-06-19 · Sub-deploy 4 (unified pipeline) — 2026-06-19 · Sub-deploy 5 (8.6 Quote Builder + Deposit + 8.7 Quote Edit) — 2026-06-21 · Sub-deploy 6 (8.8 Orders Tickets full-page detail) — 2026-06-22 · Sub-deploy 7 (8.9 Stock accuracy + Orders pipeline enforcement) — 2026-06-23 · Sub-deploy 8 (8.10 Orders screen read-only + Confirm Order in Sales Ticket) — 2026-06-23 · Sub-deploy 9 (8.11 Send Quote to customer) — 2026-06-23 · Sub-deploy 10 (8.12 Reseller order cart restoration) — 2026-06-29 · 8.38 Samples Account — 2026-07-15 · 8.45 Notification Escalation & Digests — 2026-07-28 · 8.56 Customer/Reseller Proof of Payment Upload — 2026-08-21 · Bug fix — Cancelling a Sales ticket now cancels its linked Order Ticket (packing board entry) too, so it no longer lingers on the Operations Monitor after the order it tracked is dead — 2026-09-07 (see `CLAUDE.md`'s Order pipeline business rules and `ticket_routes.py::_cancel_linked_packing_board` for the mechanics) · New Direct Inquiry now routes straight into the created ticket's detail page (Build Quote one click away) instead of reloading the bare list — 2026-09-18 (`SalesTickets.js::createTicket`) · Quote Builder draft persistence — 2026-09-20: a long hand-built quote previously lived only in React state, lost on an accidental refresh or an inactivity-logout redirect (`api.js`'s 401 interceptor does a hard `window.location.href` reload). Now autosaved to `localStorage` (debounced ~400ms), keyed `bassani_quote_draft_${userId}_${ticketId}` — same proven pattern as the reseller/customer cart's `bassani_cart_${userId}` (confirmed logout only ever clears the `token` key, so drafts survive a logout/re-login cycle). Create-mode (`openQuoteBuilder`) silently restores with a toast, since there's no competing source of truth before an Odoo order exists; edit-mode (`openQuoteEdit`) shows a Restore/Discard banner instead, since the freshly-fetched live Odoo order lines are a real competing source of truth. A restored draft's lines are re-validated against current Odoo product data (`GET /api/products/?ids=`) before use — same "never trust a stale price" principle already established by the Reorder feature (`Views.js`, 2026-08-21) — dropping any product no longer catalog-visible with a toast. Cleared on a successful save; a "Discard Draft" toolbar action is also available once a draft has been applied. Frontend-only (`SalesTickets.js`), no backend changes. **Same gap found and fixed in the reseller/customer cart, same day:** the cart's own restore-on-refresh effect (`Views.js::Orders()`, built 2026-08-21, predates and was the precedent for the quote-builder work above) restored a saved cart's line items exactly as cached — no re-check against Odoo — and `order_routes.py::create_order` writes whatever `price_unit` the client submits straight into the order line with no server-side re-pricing. So a cart resurrected from localStorage after sitting untouched (a price changed, or a product was archived) could silently submit a stale figure with zero safety net — unlike Reorder, which already re-fetches. The restore effect now does the identical live re-fetch (`GET /api/products/?ids=`) before restoring, dropping any product no longer catalog-visible with a toast, matching Reorder's own established behavior; falls back to the cached cart (unvalidated) only if the re-fetch itself fails (e.g. a network blip), rather than losing the cart outright — it still goes through the normal stock-check/confirm flow before anything reaches Odoo either way.  
...
**Superseded 2026-08-27 (8.58):** this design decision was reversed. Every quote/invoice send (this one included) now goes through the portal's own Resend-based `email_service.py` instead of Odoo's `mail.template` — Odoo's own mail delivery on this instance proved unreliable and undiagnosable from the portal side (mail could silently fail to arrive with no error surfaced back to the API call that triggered it), and the emails never carried portal branding or followed the Email Standards. See `CLAUDE.md`'s `email_service.py`/`ticket_routes.py`/`invoice_routes.py`/`packing_board_routes.py` entries for the current mechanics, including the 2026-08-28 fix that restored the paid/unpaid-with-banking-details wording Odoo's original template had (checked live against Odoo's actual `mail.template` records and found the portal's rewrite had dropped that content, not just the branding).
...
**State propagation (original, superseded 2026-08-23 — see below):** `detail?.order_id + deliveries.some(d => d.is_backorder)` triggers MO fetch in SalesTickets; `detail?.status === "waiting_stock"` triggers MO fetch in OrdersTickets. Both degrade gracefully when no MOs exist.

**Fixed 2026-08-23 — visibility gap found while auditing cross-dashboard sync:** an MO routinely exists well before any backorder forms (Odoo often creates one the moment the sale order confirms — the whole reason `ManufacturingOrders.js`, 23.5, was built), so the Operations Monitor's `has_mo_pending` badge (23.4) could already show "in production" on a ticket's card while the ticket-level screens a Sales rep or Packer actually works from showed nothing at all — the monitor board was ahead of the working screens. Both fetches now trigger as soon as `order_id` exists, with display gated only on `mos.length > 0`, not on backorder/waiting_stock state. `OrdersTickets.js`'s panel was also promoted out of the waiting_stock-only amber card into its own standalone card (mirroring `SalesTickets.js`'s), so it's visible for a `queued`/`packing` entry too, not only once it's already a backorder.
...
**Addendum, 2026-08-23 — visual redesign for ERP parity:** the page had accumulated real duplication — the pipeline stepper, a separate status-pill rail, and each card's own status text were all independently re-deriving the same underlying order/ticket/packing/invoice state. Rebuilt as a hero card (order ref, one canonical status badge, customer, four `StatCard` KPI tiles — Order Total / Outstanding / Items / Fulfilled %) above a two-column body: main column (a new `TimelineCard` replacing both the pipeline stepper and the status-pill rail, then Order Lines with product thumbnails, then Delivery & Fulfilment) and a `lg:sticky` sidebar (Sales Ticket, Packing, Proof of Payment, Invoice(s), More Actions). The `TimelineCard` is a dated, chronological, icon-node vertical timeline built strictly from fields the API already returns — it never fabricates a timestamp, and stops at whichever step actually halted the order rather than showing dead pending steps past a cancellation. `GET /api/orders/{order_id}/passport` gained a `product_images` map (`{product_id: image_128}`, same batched shape as the existing `lot_map`) specifically to back the new order-line thumbnails, since `sale.order.line` itself carries no image. No functional/permission changes — every existing action, conditional, and role gate carried over unchanged; this was a presentation-layer rebuild only.
...
**Addendum, 2026-08-21 — third digest added:** `run_mo_digest` (`scheduler.py`) is a third 17:00 SAST digest, same shape as the two above, listing every Manufacturing Order still in progress (`mrp.production`, `state not in ["done","cancel"]`) that Odoo's own replenishment routing created for a backordered customer order — scoped via `origin like "S0"` to exclude the unrelated vault/manicuring-lab MOs. New `mo_daily_digest_to` routing key, `send_mo_daily_digest` in `email_service.py`, new `ROUTING_KEYS` entry ("Daily Digest: Production Orders (MOs)"). Prompted by there previously being no notification at all when a new MO appeared — staff only discovered one by opening the ticket/Order Passport. `EmailRoutingConfig` is now 17 fields, `ROUTING_KEYS` 18 entries (17 configurable + the 1 preview-only `recurring_order_upcoming` from Phase 8.46).
...
**Follow-up, 2026-08-27 — in-portal review for customer logins, admin Run Now, and two real bugs fixed along the way.** Raised as a direct question: a customer with a portal login had exactly the same experience as one without — the emailed token link was the only way to accept/decline, which is inconsistent with Architecture Principle #2 ("the portal is the main point of access"). Implemented rather than deferred:
- [x] Accept/decline logic extracted out of `public_routes.py` into shared core functions in `recurring_order_routes.py` — `occurrence_already_actioned()`, `_accept_occurrence_core(ticket, actor, background_tasks)`, `_decline_occurrence_core(ticket)` — so the public token path and the new authenticated path can never drift on what "accept"/"decline" actually do. `actor` is a parameter: the public path still passes the synthetic system actor; the in-portal path passes the real `current_user`, so `_confirm_order_core`'s own ownership check applies for free and `stage_history` attributes the real person instead of "system."
- [x] New customer-only endpoints (`_require_recurring_review_access`): `GET /api/recurring-orders/mine/pending`, `POST /api/recurring-orders/mine/{ticket_id}/accept`, `POST /api/recurring-orders/mine/{ticket_id}/decline`. Reseller not included — the recurring notice email itself only ever goes to the end customer, never a reseller, so there's no reseller-facing occurrence to review yet.
- [x] `OrderPassport.js` — `isPendingRecurringReview` drives a full-width "This is your regular recurring order" card directly under the hero, with Accept/Decline, replacing the generic Confirm Order/Cancel Order toolbar buttons for that state (Edit Quote stays available — confirmed it never touches `ticket.status`, so revising before deciding still works). `get_order_passport`'s ticket projection gained the fields needed to drive this (`scheduled_for`/`customer_accepted_at`/`customer_declined_at`/`needs_manual_confirm`/`manual_confirm_reason`).
- [x] `Dashboard.js` — customer-only banner surfacing any pending occurrence(s), linking through to Order Passport rather than duplicating Accept/Decline inline (Order Passport stays the one place that logic lives).
- [x] `RecurringOrders.js` (admin) — **Run Now** action per active schedule: manually fires `_generate_one_occurrence()` immediately, bypassing the 2-days-out window check. Two purposes: testing the generate → review → accept/decline chain without waiting for real dates to line up, and ops recovery if the daily 08:00 SAST job is ever missed (same class of incident as the Sales mailbox Graph subscription lapse, 11.C). Not a preview — creates a real draft Odoo order and emails the real customer.
- [x] **Bug found and fixed:** `needs_manual_confirm` (set on a credit-limit block during accept) was never cleared once a staff member resolved it via a normal manual confirm — `_confirm_order_core`'s ticket update only ever `$set` `status`/`stage_history`. `RecurringOrders.js`'s occurrence badge (which checks this flag before `customer_accepted_at`) permanently showed "Needs manual confirm" on an already-confirmed order. Fixed by `$unset`-ing both fields the moment the ticket reaches `awaiting_deposit` by any path.
- [x] **Second, more serious bug found investigating the above:** the draft order behind a pending occurrence was never actually protected from the normal portal UI — since it's a real order the moment it's generated, a customer could confirm/edit it via the ordinary Order Passport buttons before ever opening the email, which advanced the ticket's status but never touched the three fields the token endpoint checked. That left the token "valid" afterward, and a later Decline click hit a blind `action_cancel` with no order-state check and a swallowed exception — the portal ticket could get force-marked declined while Odoo silently refused the cancel and the order kept progressing, a real desync with the financial source of truth. Fixed by `occurrence_already_actioned()` rejecting on `ticket.status != "quote"` (not just the three recurring-specific fields), and by no longer swallowing a failed Odoo cancel in `_decline_occurrence_core`.
...
#### 8.56 — Customer/Reseller Proof of Payment Upload — Complete 2026-08-21

**Goal:** Once an order is confirmed, a customer (or reseller) had no way to hand Finance proof they'd paid the deposit or balance except email/WhatsApp outside the portal. This adds a self-service upload on the order's own Order Passport page, visible to Sales/Finance as a badge on the Sales Tickets list plus a real-time email, so Finance knows to reconcile it.

**Key decision:** the upload is evidence and a trigger only — it never registers a payment itself. Finance still explicitly clicks the existing Register Deposit / Register Balance Payment action afterward, preserving the "no exceptions" deposit gate (8.47) and the actual bank-reconciliation-based verification the portal already relies on. The customer-facing copy is explicit that the step is optional, so it never reads as blocking the order.

- [x] `backend/routes/ticket_routes.py` — new `_require_ticket_uploader` auth dependency, the first customer-role branch anywhere in this file (every existing helper — `_require_ticket_viewer`/`_require_ticket_driver` — only ever passed through `role == "reseller"`); pass-throughs `role in ("reseller", "customer")`, staff fall back to `tickets.sales`/`tickets.finance_confirm`. New `_assert_ticket_uploader_owns_ticket()` mirrors `_assert_reseller_owns_ticket` for reseller, adds the `customer_company_partner_id` equality check for customer (same pattern used throughout `order_routes.py`/`recurring_order_routes.py`'s customer branches).
- [x] `POST /api/tickets/{id}/pop` — multipart upload, mirrors `customer_routes.py::upload_customer_document`'s R2 shape exactly (`r2_put`, key `tickets/{id}/pop/{upload_id}{ext}`). Rejects a closed ticket. `$push`es onto new `tickets.pop_uploads[]` array (`id, r2_key, filename, size, uploaded_at, uploaded_by_name, note`), sets `pop_awaiting_review: true`, audit-logs, fires `send_pop_uploaded_notification` (non-blocking) to the `pop_uploaded_to` routing key.
- [x] `GET /api/tickets/{id}/pop/{upload_id}/download` — fresh `r2_presign` URL generated on demand (never baked into the main ticket payload, since presigned URLs expire).
- [x] `POST /api/tickets/{id}/pop/mark-reviewed` — clears `pop_awaiting_review` without registering a payment, for the duplicate-upload/handled-another-way case. Gated `tickets.finance_confirm`, same permission as the payment actions — no new permission added, so no `Users.js` changes needed.
- [x] All four of the ticket's existing payment-confirming code paths (`register_deposit`, `register_balance_payment`, `use_existing_invoice`, the Odoo-state `confirm_payment` sync) now also clear `pop_awaiting_review: false` on success — the natural "resolved" transition, no separate staff click needed when a payment is actually registered.
- [x] `backend/routes/order_routes.py::get_order_passport` — `pop_uploads`/`pop_awaiting_review` added to both the ticket Mongo projection and the curated `ticket_out` response dict (this endpoint hand-builds its response rather than returning the raw ticket doc, unlike `list_tickets`/`get_ticket` which needed no changes at all since neither applies a field projection).
- [x] `backend/services/email_service.py` — new `send_pop_uploaded_notification`; `backend/routes/settings_routes.py` — new `pop_uploaded_to` `EmailRoutingConfig` field + `TEST_EMAIL_SENDERS` entry; `frontend/src/views/EmailSettings.js` — new `ROUTING_KEYS` entry ("Finance: Proof of Payment Uploaded").
- [x] `frontend/src/views/OrderPassport.js` — new "Proof of Payment" card: always shown to reseller/customer once there's an active (non-closed) ticket, with an explicit **this is optional** line and an Upload button (hidden `<input type="file">`, same interaction pattern as `CustomerProfile.js`/`PublicDocUpload.js`); shown to everyone else (staff included) only once something has actually been uploaded, read-only. Lists prior uploads with a "View" action that fetches a fresh presigned URL on click.
- [x] `frontend/src/views/SalesTickets.js` — list view's Payment column gains an independent amber "POP Uploaded" badge (`t.pop_awaiting_review`), shown alongside (not replacing) the existing green "Confirmed" badge — a ticket can have its deposit already confirmed while a *new* upload for the balance is still awaiting review. Detail view gains a compact summary banner (file count, latest filename, deep link to the full list on Order Passport rather than duplicating that presigned-URL-fetching UI a second time) plus a **Mark POP Reviewed** action button (`tickets.finance_confirm` only) alongside the existing Register Deposit/Register Balance Payment actions.

### Definition of Done
- [x] A customer or reseller can upload a file against their own confirmed order from Order Passport, with clear "this is optional" copy
- [x] Staff with `tickets.finance_confirm` see an amber badge on the Sales Tickets list and receive an email the moment a file is uploaded
- [x] Uploading never registers a payment — Finance always still explicitly clicks Register Deposit/Register Balance Payment
- [x] The awaiting-review flag clears automatically the moment a payment is actually registered through any existing payment-confirming path, or manually via Mark POP Reviewed for the no-payment-needed case
- [x] A reseller/customer cannot upload against a ticket they don't own; a closed ticket rejects the upload
- [x] No new permission was introduced — reuses `tickets.sales`/`tickets.finance_confirm`
...
### 23.2 — Onboarding Pipeline Monitor — Complete 2026-08-21 (columns merged 2026-08-22)
...
**Key decision:** an application only counts as complete once the customer profile actually exists in Odoo (`status: "approved"`) **and** the welcome pack has actually been sent (`welcome_pack_sent_at` set) — not either alone. This directly surfaces the known 8.55 failure state (approval succeeded, welcome pack send failed) via a `needs_retry` flag on the card rather than letting it silently read as "done" the moment `status` flips to `approved`.

**Deliberate scope trims vs. 23.0/23.1:** no WebSocket live-push (an application's stage doesn't change fast enough to justify wiring a refresh-broadcast into every mutation across `onboarding_routes.py` and `public_routes.py`'s customer-signing endpoint — the 30s poll fallback is the only refresh mechanism); no generalised multi-monitor Settings UI at first — reversed the next day (2026-08-22) once a second monitor genuinely existed and more were confirmed planned, see the Settings-consolidation note below.
...
- [x] Five Kanban columns: Pending Review, Docs Generated, Awaiting Signature, Countersigning (folds the frontend's `needs_countersigning`/`countersigning_in_progress` split into one column), **Ready to Finish** (approved-but-not-yet, or approved-with-pack-send-failed — see 2026-08-22 note below)
...
- [x] Age-tier deadlines: Pending Review reuses `scheduler.py::check_stale_applications`'s real existing 4h escalation threshold verbatim, so the column's "overdue" state agrees with the escalation email that already fires at that point. Every other column's deadline is a proposed first-pass default, not confirmed business policy — flagged for the product owner to tune once real usage shows what's actually urgent.
- [x] **2026-08-22 — Awaiting Signature deadline fixed to the real 30-day signing-session token lifetime:** raised by the product owner ("we generate the link and make it valid for 30 days... they essentially have up to 30 days to sign"). The column's original 72h proposed default was wrong, not just conservative — it would flag an application as overdue while the customer's actual signing link was still perfectly valid for weeks. `_DEADLINES["awaiting_signature"]` changed from 72 to 720 (30 days), and `_stage_clock()` now anchors this column's countdown to `signing_session_generated_at` rather than `signing_session_sent_at` — `onboarding_routes.py`'s `/generate-signing-docs` computes `expires_at = now + timedelta(days=30)` at generation time, so generation, not send, is when the real clock actually starts. This is the second genuinely confirmed deadline on this board (alongside Pending Review's 4h), not another first-pass estimate.
- [x] `frontend/src/views/OnboardingMonitor.js` — same full-screen dark-theme shell as `OrderMonitor.js` (30s poll, 1s live countdown tick, LIVE/OFFLINE indicator, invalid-token/loading states, no `ProtectedRoute`/`AppLayout` wrapper), 5-column Kanban + 4+7 KPI strip, cards link to `/applications/{id}` (`CustomerApplicationDetail.js`)
- [x] `<Route path="/onboarding-monitor" element={<OnboardingMonitor />} />` added to `App.js` (public, outside `ProtectedRoute`)

**2026-08-22 — Ready to Approve / Welcome Pack Pending merged into one "Ready to Finish" column:** raised by the product owner directly: since 8.55 merged "Approve" and "Send Welcome Pack" into a single button for the normal case, having two separate board columns for what's now a single action no longer matched the real workflow. Merged into one `ready_to_finish` column server-side (`onboarding_monitor_routes.py`), but the underlying distinction is preserved, not lost: the 8.55 retry case (approved, but the pack send specifically failed — a genuine error state, not just an untouched queue item) is flagged via a `needs_retry: true` field on the card, its own KPI count, and a pulsing "RETRY: WELCOME PACK" badge on the card itself (`OnboardingMonitor.js`) — so it stays visually distinct within the merged column rather than disappearing into the general queue. Clock/deadline logic per underlying case (first-approval vs. retry) is unchanged, only the display bucketing merged.

**2026-08-22 — "Awaiting Docs" column removed from the board:** in the same conversation, the product owner walked through the actual live onboarding process end to end (customer applies via `/apply` → Bassani reviews CIPC/supporting docs → generates NDA/SOA → customer signs → countersigned → Approve & Send Welcome Pack → complete) and confirmed the reseller-inbox-initiated intake path (`status: "awaiting_docs"`, created when a reseller emails in a customer's documents via the Onboarding Inbox mailbox rather than the customer using `/apply`) is not part of the live process — "that way is null and void, only way is for customer to sign up via the link." That column would therefore always read empty on the board. Removed from `_COLUMN_KEYS` and the frontend `COLUMNS`/KPI-row config; `_derive_stage()` still returns `"awaiting_docs"` as a real stage and `_DEADLINES` still has an entry for it (the underlying reseller-inbox feature and its data model are untouched, per the product owner's explicit choice to scope this as a display-only change) — an application in that stage is simply excluded from the board entirely, the same as a rejected one.

**2026-08-22 — Settings tab consolidated:** `MonitorSettings.js` and `OnboardingMonitorSettings.js` (two separate Settings tabs) replaced by a single `frontend/src/views/MonitorDisplaysSettings.js`, config-array-driven (`MONITORS`) so a future monitor (23.3) is one array entry, not a new tab or file. `Settings.js`'s tab list collapsed to one `{ key: "monitor-displays", label: "Monitor Displays" }` entry, with the two old tab-key query params (`monitor-display`, `onboarding-monitor-display`) still redirecting correctly for any stale bookmark/link.
...
- [x] An approved application whose welcome pack send failed shows in Ready to Finish with a visible RETRY badge, not silently as done
...
**Manufacturing Orders admin page (2026-08-22, same-day follow-on):** the product owner tested the new monitor and asked why it showed "many" MOs while the existing Backorders admin page showed only one — investigating surfaced a real gap, not a bug: Backorders only shows delivery-level shortfalls that already exist as an Odoo backorder picking (a packer has already attempted and split a delivery), while an MO can exist well before that, since Odoo often creates it the moment the sale order is confirmed depending on the product's routing. There was no logged-in, searchable way to browse open MOs directly — only the public TV board (no search/filter, no login) or a per-order fetch in `OrderPassport.js` that was fetched but never rendered. Closed with a new **Manufacturing Orders** nav item (Orders section, `orders.view`, same audience as Backorders):
- [x] `GET /api/orders/manufacturing-orders` (`order_routes.py`, registered before `/{order_id}` per this file's literal-path-routes discipline) — same domain as the monitor/digest, flat list (not picking-first), same batch-resolve pattern (sale order by `origin`, portal ticket by `order_id`) already used by `/backorders`. Deliberately a fourth near-identical MO-fetching block rather than a shared helper — consistent with the parallel-files preference already established for the three monitor files this session, and CLAUDE.md already tolerates the first two of these blocks having drifted field-naming with an explicit "not something to fix now" note
- [x] `frontend/src/views/ManufacturingOrders.js` — uses the shared `DataTable` component (unlike `Backorders.js`'s hand-rolled table, since this dataset is naturally flat, one row per MO, no per-picking nesting needed). State filter chips, text search, stats row — same interaction shape as Backorders. Imports `MO_STATE_LABEL` from `Backorders.js` (now exported) so the two pages can never disagree on state labels
- [x] `OrderPassport.js` gained a "Manufacturing Orders" toolbar button, shown only when `manufacturing_orders.length > 0` — this is the first thing to ever actually use that field, which was being fetched into that page's state and silently discarded before now. Deep-links with `{ state: { soName: order.name } }`, identical pattern to the existing Backorders button on the same toolbar
...
- [x] The Manufacturing Orders nav item is visible to any `orders.view` user, the page loads real data with working state filter and search, and a row with a linked sale order opens the correct Order Passport
- [x] Order Passport's Manufacturing Orders button only appears when that order actually has open MOs, and pre-filters the new page to that order's reference
...
- [x] **Same-day follow-up:** with only Confirm/Complete, testing surfaced that a portal-driven MO jumped straight from `confirmed` to `done` in one atomic request (Mark Complete writes the full `qty_producing` and calls `button_mark_done` together) — Odoo's own computed `progress`/`to_close` states were technically passed through internally but never rested long enough to show on a 30s-poll monitor, so the Manufacturing Monitor's In Progress/To Close columns only ever reflected production recorded directly in Odoo, never portal activity. Closed with a third endpoint, `PUT /manufacturing-orders/{mo_id}/record-production` (body `{qty_producing}`) — writes a partial quantity without finalising, rejects a value that doesn't strictly increase over what's already recorded or that exceeds the order quantity, same re-read/re-verify/audit-log discipline as the other two. Odoo derives `progress`/`to_close` from the write itself; the endpoint returns the resulting state for the frontend toast
- [x] `frontend/src/views/ManufacturingOrders.js` — row actions gated on `can("orders.manufacturing_manage")`. Originally shipped as three separate actions (Confirm / Record Production / Mark Complete); Confirm has no stock side-effect and fires directly (draft rows only).
- [x] **Simplified to two actions, same day:** feedback was that the primary interaction should just be "change the status," not choosing between three actions. Record Production and Mark Complete were collapsed into one **Update Status** button + one modal — the quantity input defaults to the *full* order amount (so the common "it's all fully produced" case needs zero typing, just open and save), and the frontend decides which endpoint to call based on the entered value: `qty >= qty_total` calls `/complete` (which already auto-tops-up server-side regardless of what's passed), anything less calls `/record-production`. The modal's inline warning and Save button label ("Mark Complete" vs "Save Progress") react live to the typed quantity, replacing the old separate confirmation-modal-on-top-of-a-button pattern with one inline warning inside the single modal instead
- [x] **Automatic backorder recheck (same day):** both `/record-production` and `/complete` now call a shared `_check_and_notify_backorder_stock()` (extracted from the existing manual "Check backorder stock" button in `packing_board_routes.py`, invoked via a deferred import from `order_routes.py::_recheck_backorder_stock()` to avoid a module-load cycle) immediately after a successful write — production advancing is exactly the event that should prompt Odoo's reservation state to be rechecked, not just wait for a manual click. Best-effort: never fails the MO action if the recheck itself errors. Beyond the existing reseller/internal email, this now also pushes a real-time notification (`notify_ticket_handoff`, reusing its existing "a ticket I own changed stage" semantics) to the specific staff member assigned to the Sales ticket whose backorder just cleared — not just a routing-list email, an actual answer to "will the person handling this ticket be told"
...
- [x] **Confirmed unchanged, asked directly by the product owner:** a backorder cleared by MO completion still goes through the full normal pipeline — pack, QA, RP, mark_complete, invoice — exactly like any other delivery. Nothing about the MO actions bypasses that gate; they only ever touch `mrp.production`, never `stock.picking`/QA/RP state directly
...
**Entry-identity fix, same-week follow-up (2026-08-23) — the most significant of the three:** re-queuing a reactivated backorder (the fix above) immediately surfaced something more serious. Every per-entry `packing_board` endpoint — assign, tick, mark-packing, mark-ready, QA/RP approve, complete, incomplete, cancel, override-status, assign-lot, and the detail fetch itself — looked a record up by `order_id` alone. Safe only when exactly one entry exists per order, which stops being true the moment a reactivated backorder and its already-finished primary coexist under the same `order_id` (both now visible as separate rows). A bare lookup had no way to tell them apart; an action meant for the backorder could silently land on, and corrupt, the already-`"complete"` primary instead, with nothing on screen indicating it happened. Confirmed with the product owner before starting: the trained click-through flow could not change at all — this is fixed entirely in the request payload, invisibly.
- [x] `backend/routes/packing_board_routes.py` — new shared `_entry_query(order_id, picking_id)`, generalising a pattern that already existed in exactly one place (`mark_collected`'s optional `picking_id`, defaulting to "the primary, non-backorder entry" when omitted) to every other endpoint. Backward compatible by construction — an order with no backorder only ever has one entry matching either branch, so omitting `picking_id` behaves identically to before
- [x] `frontend/src/views/OrdersTickets.js` — `picking_id: detail.odoo_picking_id` threaded into every mutating call and both `GET /entry/{order_id}` fetches; `detail.odoo_picking_id` was already present on every entry (used by the existing Print Packing Slip button), so this needed no new state or fetch. Zero visible change — every button, screen, and step is identical to before
- [x] Two further real, independent bugs found and fixed while auditing every affected call site: `complete_entry`'s own final status-write had a hardcoded `is_backorder: {"$ne": True}` filter regardless of which entry the function had actually resolved and validated earlier in the same request — completing a backorder's own delivery would have silently tried to re-complete the primary instead; `assign_lot` searched Odoo for every active picking on the sale order rather than the one specific picking, which could match move lines on the wrong delivery once two pickings exist for the same order
- [x] Separately flagged, not fixed (unrelated to this fix, found during the same audit): `OrdersTickets.js::savePacker()` calls `PUT /api/packing/assign-packer`, but the only registered route is `PUT /api/packing/assign` — a pre-existing 404
...
- [x] **Default-collapsed except the first row (2026-08-23, third follow-up):** every group now starts collapsed except the first row on the current page, so the page reads as a scannable summary by default rather than a wall of expanded product rows
- [x] **Backorders.js parity, same day, deliberately partial:** `Backorders.js`'s By Order view gained the same Age column (backed by a new `create_date` field added to `/api/orders/backorders`'s `stock.picking` read) and the same always-visible, default-collapsed-except-first chevron pattern. Rollup status and SO-grouping were confirmed NOT needed there — a backorder is a single `stock.picking` with one state covering every line, unlike an MO group's several independent, potentially-disagreeing `mrp.production` records — and the page's read-only nature was confirmed intentional (the real remediation actions already live on the Manufacturing Orders page and the Packing Board)
- [x] **Per-product rows, same-day follow-up:** expanding an order now renders each product as its own table row underneath (matching `ManufacturingOrders.js`'s child-row layout) instead of stacking every product inside the parent row's single Outstanding cell. That cell now always shows a fixed total-outstanding + product-count summary regardless of expand state
...
- [x] `frontend/src/views/ManufacturingOrders.js` — added the shared `Pager` component under the By Order table (same client-side pageIndex/pageSize/total contract already used by the production module's four paginated tables), resetting to page 1 when the filtered result set's shape changes. By Product already had pagination built into `DataTable` and needed no change
- [x] New summary KPI row (Orders Affected / Units Remaining / Oldest Open) above the existing state-breakdown row, mirroring the public Manufacturing Monitor's own KPI row — computed client-side from fields already present in the payload, no backend change. Orders Affected is the distinct-order count this page was specifically missing
- [x] Fixed a same-round bug found while touching this code: `orderGroups`'s sort comparator ranked each group by its `Math.min` (newest) member's age instead of `Math.max` (oldest/most urgent) — the opposite of the "never bury an urgent product" behaviour the surrounding comment already documented
...
- [x] Recording a partial quantity on a confirmed MO moves it into Odoo's `progress` state (or `to_close` once fully recorded) and the change is visible on the public Manufacturing Monitor within one poll cycle, without finalising the order; entering a value that doesn't increase or that exceeds the order quantity is rejected with a clear message, both client-side and server-side
- [x] The Manufacturing Orders page shows exactly two actions per row (Confirm on draft; Update Status otherwise) — opening Update Status and saving without changing the pre-filled quantity marks the order complete in one step, with no separate confirmation screen
- [x] Marking an MO complete (or recording enough partial progress) that satisfies a `waiting_stock` backorder automatically clears that backorder's flag, sends the existing reseller/internal email, and pushes a real-time notification to the assigned Sales rep, without anyone clicking "Check backorder stock" manually
- [x] That backorder still requires its own pack → QA → RP → mark_complete → invoice cycle before reaching the customer — confirmed unchanged by this work
...
- [x] **Same-day follow-up:** every Manufacturing Monitor group now shows its order reference + customer heading, including single-product orders — previously only groups with 2+ products got a heading at all, so a single-item order's card had no prominent order/customer context
- [x] **Order-focused KPI strip, same day:** replaced the headline per-MO KPIs (Overdue/At Risk/In Progress/To Close, counting individual products) with order-level rollups — **Orders Overdue** / **Orders At Risk** / **Orders Affected** / **Oldest Order Waiting** — computed client-side from the already-fetched cards (worst-tier-wins per order: an order is overdue if any one of its products is). No backend change. The original per-MO breakdown moved to row 2 as a secondary production-floor signal
...
### Definition of Done
- [x] Each of the three boards loads in dark theme by default with no stored preference, and the toggle switches to light and persists across a refresh (per browser, via `localStorage`) — flipped from an initial light default the same day, per direct product-owner feedback after seeing it live that dark suits a TV/NOC-style board better
...
- [x] **Multi-company logins (2026-08-21):** the reverse case — one email linked to *several* companies (a person who's a contact on more than one, same "multiple branches" pattern as onboarding) — previously hard-failed provisioning with "An account already exists," forcing a second separate login/password. `users` now holds `companies: [{odoo_partner_id, customer_company_partner_id, company_name, active}]` + `active_company_partner_id` (mirrors `active_warehouse_id`) instead of the single fixed fields above; those two field names are kept as **derived, always-fresh** values (`auth.py::resolve_customer_active_company()`, called from `get_current_user()` and from `login()`/`verify_otp()`) so every existing order/invoice/dashboard/recurring-order/warehouse-scoping call site needed zero changes. New `PUT /api/users/me/active-company` + `CompanySwitcher` (`UI.js`, mirrors `WarehouseSwitcher`) let a customer switch companies from one login, shown only once there's more than one. `grant_portal_access` now adds a company to an existing customer login by email instead of failing (no second password, a distinct "company added" email), surfacing the overlap to the admin ahead of time via `linked_elsewhere` on `get_portal_access` with a confirm step in `CustomerProfile.js`. Per-company revoke (`deactivate_portal_access`/`reactivate_portal_access`) now toggles one `companies[]` entry, not the whole login. See `CLAUDE.md`'s "Multi-company customer logins" note for the full mechanics.
...
- [x] **Product visibility (fixed 2026-08-21):** a customer must abide by the exact same rules as a reseller — restricted to the curated `reseller_catalog` allowlist, not the full Odoo product catalog. `product_routes.py`'s three reseller-only gates (`list_products`, `list_categories`, `get_product_by_barcode`) were keyed literally on `role == "reseller"`, which silently gave a `customer` login the entire unrestricted catalog until caught and fixed to `role in ("reseller", "customer")`
- [x] **Self-service order confirm (fixed 2026-08-21):** a customer can confirm their own placed order — same trigger for the automatic pro-forma-invoice-for-50%-deposit email every other confirm path already fires. `_require_confirm_access` (`order_routes.py`) previously allowed only staff-with-permission or reseller through, so a customer got a flat 403 with no way to ever move their own order past draft. Fixed by adding `customer` alongside `reseller`, plus a matching ownership check inside `_confirm_order_core` (single `customer_company_partner_id` equality, same simplification as every other customer branch). Surfaced from the customer's own Orders detail page (not the Sales Ticket pipeline, which stays staff/reseller-only) via a real "Confirm Order" action — `OrderView.js`'s existing (previously unwired everywhere) confirm button, given the same stock-check-first flow SalesTickets.js uses for staff/resellers, minus the credit-override option (a customer over their credit limit is told to contact Bassani, not offered a self-override)
- [x] **Also fixed the same day, found while wiring the above:** `GET /orders/{id}/stock-check` was gated by `require_permission("orders.read")` — not a real permission key, and `require_permission` rejects any role outside the admin/ticket-role sets outright, which already included `reseller`. So this endpoint silently 403'd for every reseller too (not just the new customer role) — invisible because the frontend's `catch` swallowed the failure and skipped straight to confirming with no stock-check modal ever shown. Fixed by reusing `_require_confirm_access` as the dependency (moved earlier in the file so the reference resolves) plus the same per-order ownership check, since the endpoint is only ever called immediately before confirming that same order
- [x] **Proforma-invoice reseller CC now ownership-based, not placement-based (2026-08-21):** `_confirm_order_core`'s automatic pro-forma-deposit email previously CC'd the reseller only when `sales_ticket.reseller_id` was set — true only for orders a reseller physically placed themselves. A customer's self-placed order never sets that field, so a linked reseller (who still earns commission on it) silently never got CC'd. Fixed to key off `_owning_reseller_id` (`get_owning_reseller_id`, `customer_ownership`) — the exact same lookup the commission-crediting block a few lines above already uses — so the CC now follows "who earns commission on this," consistent with everything else Phase 7.13 established. The now-dead `_ticket_reseller_id` local (its only remaining use) was removed along with its stale comments
- [x] **Critical fix — customer's own order list was silently warehouse-filtered (2026-08-21, found live by the product owner testing a real customer login):** `list_orders` applies a warehouse domain filter for every role, and `resolve_warehouse_id()`'s customer branch can basically never return `None` (falls back to the global admin default). A customer's "My Orders" list was silently restricted to one warehouse, hiding any order fulfilled from a different one — including ones Bassani staff placed on the customer's behalf, directly contradicting the "every login sees the whole company's order history" promise this phase was built on. Fixed by skipping the warehouse filter for customer role in `list_orders` only (staff/reseller unaffected). `list_invoices` was never warehouse-filtered, and single-order lookups (`get_order`, `get_order_passport`) don't call `resolve_warehouse_id` at all — only the list view was ever broken
- [x] **Critical fix — customer invoice list and dashboard stat silently broken (2026-08-21, found live by the product owner):** invoices visible on the admin Customer 360 page but zero on the customer's own Invoices page. Root cause: `list_invoices`'s customer branch searched `account.move` by `commercial_partner_id` — the only place this codebase ever uses that field/model combination as a domain search (every other usage is on `sale.order`, proven, or a plain `res.partner` field read), never live-verified. Fixed by resolving `partner_id IN [company, child contacts]` instead — provably valid, already used for the reseller branch. Identical bug found and fixed a second time in the dashboard's "unpaid invoices" stat, silently swallowed by its own `try/except`. Also defaulted the reseller/customer Invoices page to "All" instead of "Outstanding" — a customer with only paid invoices landed on an empty list every time even after the scoping fix
- [x] **Cart UX audit and fixes, following a competitor/industry-standard comparison (2026-08-21):**
  - **200-product cap fixed** — `loadCartProducts` previously called `GET /api/products/` with a bare `limit: 200` and no pagination; any "All categories" browse (or one large category) past 200 products silently dropped the rest, with the endpoint's own `total` fetched but never checked. Now uses the existing `fetchAllProducts()` helper (already used by the product-export feature) instead of a raw capped call
  - **Cart persistence** — the cart is now saved to `localStorage` per user and restored on mount; previously pure in-memory state, lost on any refresh or accidental back-navigation. Cleared on successful submit; skipped while editing an existing quote or mid-reorder-prefill, so those sessions can't pollute a later fresh cart. New **Clear Cart** button (confirmation modal, matching the project's zero-`window.confirm` convention) is now the only way to explicitly empty it — `openNewOrder` no longer wipes the cart itself
  - **Delivery address selection** — the dead `OrderCreate.delivery_address` free-text field (declared, never once read anywhere in `order_routes.py`) replaced with a real `shipping_address_id`, validated server-side and written to Odoo's `partner_shipping_id`. Cart shows a "Deliver To" dropdown when the customer has saved addresses beyond their primary. `GET /{customer_id}/addresses` gained its first-ever ownership check in the same pass — it previously had none, letting any reseller/customer read any company's addresses by id
  - **Reorder** — a "Reorder" button on the Order Passport toolbar (reseller/customer) prefills a fresh cart from a past order's lines, re-fetching **current** price/stock per product rather than reusing the old order's frozen price — same "never invoice stale prices" principle the recurring-order engine already applies. A product no longer catalog-visible is dropped with a toast, not a hard failure
  - **Self-service recurring orders** — a customer can now click "Make Recurring" from their own Order Passport, not just a reseller from the ticket detail page. New `_require_recurring_setup_access` dependency (deliberately not merged into the shared `_require_ticket_driver`, which gates four other staff/reseller-only ticket-pipeline endpoints) plus a customer ownership branch in `create_recurring_order`. The passport's ticket payload gained `recurring_order_id` so the button hides once a schedule already exists
  - **Sort control** — Name / Price low-high / Price high-low, always subordinate to the existing in-stock-first ordering
  - **MOQ** — confirmed with the product owner to stay a one-time floor, not an increment/multiple-of rule; no change made
...
- [x] **Product visibility parity (fixed 2026-08-21):** a customer must abide by the exact same rules as a reseller — restricted to the curated `reseller_catalog` allowlist, not the full Odoo product catalog. Three `product_routes.py` gates (`list_products`, `list_categories`, `get_product_by_barcode`) were keyed literally on `role == "reseller"`, silently giving a `customer` login the entire unrestricted catalog until fixed to `role in ("reseller", "customer")`. Same bug, same fix, also found in `parent_category_routes.py::list_parent_categories` — a `customer` was getting the full admin management payload (raw Odoo category ids, inactive drafts, admin usernames) instead of the trimmed reseller-safe view
- [x] **Category-grouped cart browsing (2026-08-21):** when "All categories" is selected, the reseller/customer order cart groups products into sections by top-level Parent Category (the same business-curated grouping the Category/Brand-Grade filter dropdowns already use, not raw Odoo categories) with a heading and item count per section, instead of one long flat grid — the standard pattern for a catalog of this size (Amazon category pages, most B2B ordering portals). New `GET /api/parent-categories/product-groups` returns id-only membership per top-level category plus the `Uncategorised` bucket; the cart matches it client-side against the product list it already has loaded, so no duplicate product fetch. In-stock sorting is applied **within each category section**, not globally — an out-of-stock item still shows under its own category (never hidden), just ordered after what's available in that section, matching how mainstream catalogs handle it rather than burying every out-of-stock item at the very bottom of the whole page. Selecting a specific category (or actively searching) falls back to the existing flat single-list view, where grouping adds nothing. Sections are collapsible (chevron toggle, expanded by default; reversed from the initial always-expanded ship 2026-08-21 same day, once a longer real category list made collapsing worthwhile)
- [x] **Two-level sub-grouping by Brand/Grade (2026-08-21, same-day second pass):** within each top-level section, products also add a smaller sub-heading per Brand/Grade child category with matching products (e.g. Pre-Rolls's Indoor/Exotic/Greendoor/Greenhouse/Budget), rather than mixing every grade tier together in one grid — grade is a real purchasing decision for a cannabis buyer, not just a filter facet. `GET /api/parent-categories/product-groups` now also resolves each top-level category's children independently, not just the top-level's folded-in union. A top-level category with no children (a flat one, no grade tiers) is unaffected — no redundant single sub-heading. Sub-headings are not independently collapsible (only the top-level section is) to avoid over-nesting the UI. Sub-heading text repeats the parent category name (e.g. "Flower / Indoor") since a bare "Indoor" gives no clue which top-level category it's under once scrolled past the section heading.
- [x] **Sub-grouping also applies with a category already selected (2026-08-21, third pass):** originally only shown in the "All categories" view — filtering down to one top-level category via the dropdown (Brand/Grade still "all") fell back to one flat mixed-grade grid, defeating the point for exactly the category most likely to have multiple grades. `Views.js::Orders()`'s grouping logic (`cartBuildGroupView`) is now shared between both views; a `cartSelectedCategoryGroup` derivation covers the single-category case, rendering the same Brand/Grade sub-headings without the redundant top-level heading (the dropdown already shows which category is selected). Falls back to the flat list only if the selected category genuinely has no children to sub-group by. Picking a specific Brand/Grade sub-category from the second dropdown still narrows to the existing flat single-list view, unaffected — at that point the buyer has already fully specified what they want to see.
- [x] **Out-of-stock items are orderable, not hidden or blocked (2026-08-21):** removed `create_order`'s hard block on any line exceeding current stock — it's now a non-blocking `stock_warning`, since the confirm-time stock-check/backorder flow is already the real point where a shortfall gets resolved (ships-now vs backorder split; any resulting Manufacturing Order is Odoo's own automatic replenishment behavior, not something the portal creates — see the CLAUDE.md Partial fulfilment note for the full backorder-vs-MO explanation). Cart cards show "+ Add to Order (Backorder)" for out-of-stock items instead of a disabled button
- [x] **No exact stock quantities shown to resellers/customers (2026-08-21):** the order cart and `ResellerCatalog.js` previously showed the raw available-quantity number; both now show only a binary In Stock / Out of Stock badge, per explicit Bassani instruction not to disclose the actual figure to either external role
- [x] **Quantity cap removed for in-stock items too, live warning added (2026-08-21, follow-up):** found while confirming resellers/customers are actually warned before a backorder situation — the qty stepper still silently clamped an in-stock item's quantity back down to `virtual_available` with zero explanation, while the +/- buttons had no cap and let it through with zero warning either way (an inconsistency, not deliberate). Cap removed uniformly for every item regardless of stock status; a live inline note now appears on the cart line the moment requested quantity exceeds on-hand stock, replacing the silent clamp. No numbers in that message on purpose — the split ("5 will ship, 15 backordered") would hand over the exact stock figure via subtraction, defeating the binary-only display above. `create_order`'s non-blocking `stock_warning` response text is now role-aware for the same reason: reseller/customer gets generic no-numbers wording, staff/admin keeps the precise requested-vs-available detail
- [x] **Sticky section headings (2026-08-21):** the cart's category/Brand-Grade section headings now use `position: sticky` so the current section stays legible while scrolling a long catalogue instead of scrolling out of view immediately — top-level category headings stick at `top-0`, Brand/Grade sub-headings dock immediately beneath at a fixed `top-11` offset. See `CLAUDE.md`'s "Sticky category/sub-category headings" note for the two CSS traps hit along the way (padding-on-scroll-container leaving a gap behind a stuck header; a negative-margin width "bleed" interacting badly with scrollbar-gutter width).
- [x] **Mobile layout (2026-08-21):** the cart page used the same layout at every screen size, which on a phone left almost no visible product grid — an always-expanded filter row wrapped into 3-4 lines and a persistent 288px cart box permanently occupied the bottom of the screen. Rebuilt below `lg` using the standard mobile-commerce pattern: Category/Brand/Grade/Variant/Stock filters collapse behind a "Filters" bottom sheet (Sort stays inline as a compact native select); the persistent cart sidebar is replaced by a floating summary bar (item count + total + "View Order") that expands into a full-screen bottom sheet on tap, reusing the exact same cart markup as the desktop sidebar (one `cartPanel` JSX definition, mounted in both places) so there's no risk of the two ever drifting apart. `≥lg` (tablet/desktop) behaviour is unchanged — persistent sidebar, no floating bar, no sheets. See `CLAUDE.md`'s "Mobile cart layout" note for the full breakdown.
- [x] **Place Order vs Save as Draft, and recurring built into checkout (2026-08-21):** Place Order previously only ever created an unconfirmed draft quotation — actually confirming it required a separate, easy-to-miss trip to the order's own detail page. Cart footer now offers two actions: **Save as Draft** (the old create-only behaviour, renamed) and **Place Order** (primary), which chains straight into the existing stock-check-review-then-confirm flow (`GET stock-check` → review modal → `PUT confirm`) immediately after creation — reused unchanged from the standalone order-detail Confirm Order action, not reimplemented. Applies to resellers too, a deliberate reversal of the previous design where every reseller order rested at `quote` status pending Bassani staff review before confirm — confirmed with the product owner that the portal's own stock/credit checks already cover what that manual review existed to catch. A reseller/staff credit-limit block still offers "Confirm Anyway" (mirrors `SalesTickets.js`); a customer doesn't get that override, same as before. Cancelling either review modal leaves the order as a draft (it already exists by that point), never loses it. Also added a **"Make this a recurring order"** checkbox directly in the cart footer, firing `POST /api/recurring-orders` right after creation using a new `ticket_id` in `create_order`'s response — previously recurring setup was Order-Passport-only since there was no way to reach the newly created ticket's id from the cart. See `CLAUDE.md`'s "Place Order vs Save as Draft" and "Recurring order setup built into checkout" notes.
- [x] My Orders — the same `Orders()` list staff and resellers use, scoped server-side to the customer's own company; internal-only columns (Sales Ticket status, Packing status, Create Sales Ticket action) are hidden for both reseller and customer roles. Row click opens the Order Passport (25.1.2, 2026-08-25) rather than the staff `OrderView` detail; **Confirm Order** for a still-draft order lives on Passport's toolbar (2026-08-21, moved to Passport 2026-08-25) — stock-check-first, no self credit-override — since a customer has no access to the Sales Ticket pipeline UI where staff/resellers otherwise confirm
...
### 25.1.1 — Portal Login Management & Duplicate Repair ✅ Complete (2026-08-25)

**Context:** a real support case — an admin granted portal access to the same contact on two separate customer (company) profiles, expecting one login that could switch between both. The customer could only ever see one company in their switcher. Root cause: `grant_portal_access`'s existing-login check (`find_one({"username": ...})` before insert) isn't race-proof against two near-simultaneous grants for the same email on two different company profiles — only `companies.odoo_partner_id` has a unique index, `username` doesn't — so two separate `users` documents ended up sharing one email, each holding only the one company it was granted against. Diagnosed by pulling the raw document directly; there was no way to see this from the admin UI itself.

**Definition of Done:**
- [x] `GET /api/customers/portal-logins/{email}` (`customer_routes.py`) — returns every `users` document matching that email (deliberately `.find()`, not `.find_one()`), each with its full `companies` array, plus `duplicate_logins: true` when more than one comes back. Gated by `customers.manage_portal_access`.
- [x] `POST /api/customers/portal-logins/{email}/merge` — folds one login's `companies` entries into another's (skipping any `odoo_partner_id` already present) and deletes the redundant document. Audit-logged as `customer.portal_logins_merged`.
- [x] `frontend/src/components/PortalLoginManageModal.js` — shared modal: lists every company on the login with per-company activate/deactivate (reusing the existing `deactivate`/`reactivate` endpoints, no new write path for that part); when duplicates exist, shows an inline merge tool (pick which login to keep, defaulting to whichever has an actual `last_login_at` so a merge never resets a password the customer already knows; second click required to confirm, matching the codebase's zero-`window.confirm` convention via an inline confirm state rather than a nested `Modal`).
- [x] Reachable from two places with the same `email` prop: `CustomerProfile.js`'s Portal Access table ("Manage" button next to any contact whose `portal_status` isn't `not_provisioned`) and `Users.js`'s customer-role rows (`Building2`-icon button next to Reset Password) — the user explicitly asked for both, since a `users` document underlies both screens.
- [x] **"Add this company" direct action (same day, found while using the tool live on the real incident):** the normal Grant Access checkbox on a company's own Portal Access table only renders when that page's status lookup says `not_provisioned` for a contact — live testing turned up a case (Curabliss Hazelwood / Henk Krebs) where the lookup said "Active" for a company the login's real `companies[]` had no entry for at all, so the checkbox never appeared and there was no way to grant it normally. `CustomerProfile.js` now passes a `context` prop (this page's own company id + this row's contact id) into the Manage modal; if that company isn't already on the login, an "Add This Company" button calls the same grant endpoint directly with that specific contact id, sidestepping the broken checkbox gate rather than blocking on diagnosing it first.
- [x] **Root cause of the "false-positive" status, found via live Odoo + Mongo read-only comparison (same day):** it wasn't false at all — a *different* login (`henk@bassanihealth.com`, an internal-looking email, provisioned 2026-08-24) genuinely held that exact contact id, granted by mistake instead of the customer's real email (`henk@curabliss.co.za`). The table's status was accurate; what was missing was any indication it belonged to a different account, since the table only ever showed the contact's own Odoo email next to the status. Fixed at the source: `get_portal_access` now returns `linked_username` per contact so a mismatch is visible directly, and `grant_portal_access`'s existing-link check now distinguishes a same-email rematch (silent no-op, unchanged) from a different-email conflict (a clear `errors` message, previously silently swallowed as `skipped_existing` — this is also what made the "Add This Company" action from 25.1.1's earlier entry above falsely report success while doing nothing).
- [x] **`DELETE /{customer_id}/portal-access/{contact_id}`** (`remove_portal_access_link`) — the actual unblock mechanism: permanently removes one company/contact link from whichever login holds it, distinct from deactivate (which leaves the entry, and its claim on the unique `companies.odoo_partner_id` index, in place — so deactivating a wrong-account entry does *not* free it up for a correct re-grant). `CustomerProfile.js`'s Portal Access table shows a **"Remove (wrong account)"** action (confirm modal) exactly when `linked_username` doesn't match the contact's own email.
- [x] **DB-level uniqueness on `username`/`email` (2026-08-25, same day):** the actual root gap enabling both the duplicate-login race and the wrong-email grant — uniqueness had only ever been enforced at the application layer (scattered find-before-insert checks, each independently race-vulnerable). `server.py` startup indexing now adds a real unique index on `users.username` and a sparse unique index on `users.email`, each preceded by a duplicate-check aggregation (same guarded pattern the pre-existing `customer_ownership.odoo_partner_id` index already uses) so a duplicate that predates the index doesn't crash startup — it logs a loud warning and skips instead.

---

### 25.1.2 — Customer/Reseller Order Tracking ✅ Complete (2026-08-25)

**Context:** the product owner dogfooded the customer role end-to-end — placed a real order, confirmed it (it landed correctly as a sales ticket in `awaiting_deposit`, proforma emailed to customer and linked reseller) — and then had no way, from inside the portal, to view that proforma invoice again, upload proof of payment, or see any real status beyond "the order looks like an invoice." All three complaints traced back to one bug: `Views.js::Orders()`'s "My Orders" list opened `OrderView.js` (a print/invoice mockup — no timeline, no ticket/deposit status, no POP upload) for every role's row click, instead of the already-built `OrderPassport.js`, which already had a proper timeline, a Proof of Payment upload card (with the Finance alert email), invoices, and reorder/recurring actions. The list simply never linked there.

**Definition of Done:**
- [x] `Views.js::Orders()`'s `openDetail()` now routes reseller/customer row clicks to `/orders/{id}/passport` instead of the in-page `OrderView` modal; staff behaviour unchanged
- [x] Order Passport gained a **Confirm Order** toolbar action (`order.state === "draft"` only) — ported from the now-removed confirm flow in `Views.js`, including the stock-check-first modal; reseller gets the same "Confirm Anyway" credit-limit override `SalesTickets.js` offers staff, a customer does not (told to contact Bassani, unchanged Phase 25 convention)
- [x] `GET /api/orders/{order_id}/proforma-pdf` (`order_routes.py`) — same pattern as the existing `quote-pdf`/`deliveries/{id}/pdf` endpoints, fetches `sale.report_saleorder_pro_forma` via `fetch_report_pdf()`, with a reseller/customer ownership check. A **"View Pro-Forma Invoice"** action lets a reseller/customer re-view the exact document already emailed at confirm time, without digging through their inbox — lives in the Invoice(s) sidebar card (moved there from the toolbar in the same-day layout pass below), shown once `order.state` is `sale`/`done`
- [x] Proof of Payment upload was already built on Passport (8.56, 2026-08-21) but unreachable because the list never linked there — now reachable for real as soon as an order is confirmed, no backend change needed
- [x] `Views.js::Orders()`'s reseller/customer order list Status column is now role-aware: shows the real pipeline stage (packing status, else ticket status, else raw order state) in business language instead of Odoo's raw 4-state badge, which read identically as "Confirmed" for the entire multi-week deposit → packing → QA/RP → collection pipeline
- [x] "View Quote (Odoo)" relabelled "View Quotation" for reseller/customer on Passport — the "(Odoo)" suffix was leaking the internal system name to an external role with no gate at all (a real violation of the standing "never expose Odoo to customers" rule; the staff-only equivalent in `SalesTickets.js` was already correctly gated)
- [x] **Live bug found and fixed in the process:** `OrderPassport.js`'s `TICKET_STATUS_LABEL` map and `buildTimelineSteps()`'s deposit-step logic only ever recognised the ticket status `"sale_order"`, never the literal `"awaiting_deposit"` string `_confirm_order_core` actually writes on every live confirm (8.47) — so the timeline's "Deposit Registered" step rendered as already done (green check) the instant an order was confirmed, before Finance had touched anything. This is the exact order state the product owner's test order was sitting in. Both now check `["sale_order", "awaiting_deposit"]`
- [x] Dead code removed, not left behind: `Views.js::Orders()`'s customer-only confirm flow (`doConfirmOrder`, the stock-check modal, `confirming`/`stockCheckModal`/`stockCheckData` state) is gone from this file — it was only ever reachable via `canConfirmOrder={isCustomer}` inside the `OrderView`-based detail branch, which the customer role no longer renders at all once the routing fix landed
- [ ] Not addressed (flagged, not fixed): `quote-pdf` and `deliveries/{picking_id}/pdf` still have no reseller/customer ownership check at all, unlike the new `proforma-pdf` endpoint — a pre-existing gap, out of scope for this pass since it wasn't the reported issue

**Same-day layout pass (product owner feedback after seeing the first version live):**
- [x] **Sales Ticket card hidden for reseller/customer** — it only ever carried internal fields (assigned staff, internal notes, Odoo linkage); staff still see it unchanged
- [x] New **Actions** sidebar card (reseller/customer only) in its place, holding **Reorder** and **Make Recurring** — moved out of the toolbar, which had grown to 5 buttons; **Confirm Order** stays in the toolbar since it's urgent for a draft order, not a routine management action
- [x] **Pro-Forma Invoice viewer moved from the toolbar into the Invoice(s) sidebar card** — direct fix for "I see no invoices generated however there is a proforma": when no real invoice exists yet (`["sale","done"].includes(order.state)`), the card's empty state now shows the pro-forma invoice with an explanation, instead of a bare "No invoice raised yet"; once a real invoice does exist, a small link to the pro-forma stays available underneath as a historical record of the deposit-due figure. Staff view is unchanged (plain empty-state text, no pro-forma link — they already have the toolbar's document viewers)
- [x] New `HorizontalTimelineCard` component — reseller/customer see a horizontal icon-node stepper instead of the vertical dated timeline, future steps visibly greyed out, `overflow-x-auto` on narrow screens, only the current step's note shown (kept compact). Reuses the exact same `buildTimelineSteps()` output as the vertical version so the two can never disagree about order stage. Staff keep the original vertical timeline unchanged

**Same-day industry-standard audit, third round** (product owner asked what's missing from the page to meet enterprise/industry order-management standards — verified against the actual code, not guessed):
- [x] **Delivery/collection address surfaced** — `get_order_passport` now reads `partner_shipping_id`, resolves it to `order.shipping_detail` when it differs from the billing partner (i.e. the checkout "Deliver To" picker, 2026-08-21, was used), and the hero card's Meta row shows it as a highlighted "Deliver To" line; the billing address itself (`street`/`city`/`zip`, already fetched but never rendered) is now shown too
- [x] **Need Help / contact card** — new sidebar card with a `mailto:` link to `support_email` (new field on the passport response, sourced from `config.py`'s `settings.support_email`, the same address already used as the fallback recipient across the backend's own notification emails)
- [x] **Self-service cancel, draft-only** — `PUT /api/orders/{order_id}/self-cancel` (new, reseller/customer only, `_require_self_cancel_access`), same ownership checks as every other reseller/customer order endpoint, hard-blocked to `order.state === "draft"` (stricter than staff's own `/cancel`, which also allows `sent`) since a confirmed order can carry stock reservations/deposit-gate/commission implications a customer shouldn't be able to unilaterally undo. Shares `_cancel_order_core()` with the staff endpoint (same `_confirm_order_core`-style extraction) so the two can never diverge on what cancelling actually does (void commission, audit log, notify the reseller). A **Cancel Order** button (red, `X` icon) sits next to Confirm Order in the toolbar, same `draft`-only gate, with a standard confirm modal
- [ ] **Deliberately not built, flagged for the record:** courier tracking numbers (not applicable — Bassani is a collection-based business, not a shipped-parcel one; "Ready for Collection" already is the tracking event), an order-level comments/messaging thread, live auto-refresh/websocket on the page (manual Refresh judged adequate for B2B check-in frequency), a VAT/subtotal breakdown on the hero card (minor, deferred)

**Same-day, fourth round — timeline bug found testing a real order at `awaiting_deposit`:**
- [x] `buildTimelineSteps()` no longer returns early just because no packing board entry exists yet — it was silently truncating the timeline to 3 steps (Order Placed / Order Confirmed / Deposit Registered) for every order between confirm and deposit registration, defeating the entire point of a horizontal "full lifecycle, future steps greyed out" overview. Every remaining step is now always pushed with `state: "pending"` as the default until its own data appears; genuinely terminal cases (cancelled, ticket halted, packing incomplete/cancelled) still stop the list early, correctly
- [x] The horizontal timeline's Deposit Registered step, when `current`, now shows a clickable **"Upload proof of payment"** call-to-action (wired to the same uploader the Proof of Payment card uses) instead of the passive "Awaiting Finance" text — that upload genuinely is the next available action for the customer at this stage
- [x] **Voice/copy fix, applied wherever touched this session:** several customer-facing strings referred to "Bassani" in the third person ("Bassani will still confirm...", "contact Bassani directly") — this is Bassani's own system, so it should speak as "we"/"us". Fixed in `OrderPassport.js`, the cart's stock-check modal in `Views.js`, and the new self-cancel endpoint's error message in `order_routes.py`. Recorded as a standing coding-standard rule in `CLAUDE.md` (not yet swept across the rest of the codebase — email templates, onboarding, self-registration flows still need this pass)

**Same-day, fifth round — KPI tile review (product owner reviewing the hero tiles specifically):**
- [x] **Outstanding was computed wrong, not just poorly labelled** — summing each invoice's `amount_residual` reads as ~R0 the instant the deposit (only ~50% of the order) is paid, even though the other half is still owed. Fixed to `order.amount_total` minus total actually paid across all invoices, the real running balance regardless of how Odoo splits deposit vs final invoices. Now shows a `sub` caption clarifying the empty state before any invoice exists ("Shown once your deposit is registered") instead of a bare, unexplained dash
- [x] **"Fulfilled %" tile removed** — it derived from `qty_delivered` (set at Odoo pick/delivery time), so a customer could see "100% Fulfilled" while their order was merely packed and ready for collection, not yet actually collected or paid off, and reasonably read that as "done"
- [x] **Items tile gained a `sub` caption** ("All in stock" / "N backordered") — the honest version of what "Fulfilled %" was gesturing at, reusing the page's existing `hasBackorder`/`outstandingLines` data, without the false-completion risk
- [x] **Order Age tile** replaces the vacated slot — days since the order was placed, with the actual placed-on date as its caption; safe, unambiguous, can't be misread as completion
...
**Same-day, ninth round (part reverted in the tenth — see below):**
- [x] `"paid"` timeline step relabelled "Payment Received"/"Payment Pending" → **"Balance Payment Received"/"Balance Payment Pending"** in `buildTimelineSteps()` (both timelines) — avoids reading as confusable with the deposit payment already covered by the earlier "Deposit Registered" step
- [x] **Real bug, not a style tweak:** the timeline's connector divs (`flex-1`) were nested one level too deep, inside each step's own wrapper `div` — that wrapper had no `flex-grow` of its own relative to the outer row, so it sized to its content (shrink-to-fit) and the connector's `flex-1` had no leftover space to expand into. The whole strip rendered at bare min-content width regardless of container size, which is what "seems fixed width" was actually reporting. Fixed by flattening the structure so step blocks and connectors are direct siblings in the outer flex row (via `Fragment` per step) — connectors now properly absorb the row's real available width. Mobile horizontal-scroll behavior is unaffected
- [x] ~~Proof of Payment upload moved out of its own card's header into the Actions card~~ — reverted same day in the tenth round below; see that entry

**Same-day, tenth round:**
- [x] Proof of Payment reverted to its own standalone card with its own header Upload action (back to its original 2026-08-21 shape) — product owner's own call, after seeing the ninth round's version live, that it reads better standalone and that the Actions card should stay scoped to order-level actions only (Reorder, Make Recurring), not payment-adjacent ones. The timeline's own "Upload proof of payment" link on the current Deposit step was never affected by either change — always a second, independent entry point to the same upload trigger

**Same-day, eleventh round — Edit Quote:**
- [x] Confirmed a real gap: a draft order had Confirm/Cancel on Passport but no way to revise its own line items. New **Edit Quote** toolbar button (draft-only, both reseller and customer) hands off to the cart's existing `editQuote` flow — same payload shape `SalesTickets.js`'s pre-existing reseller-only Edit Quote button already used, so the cart itself needed no changes
