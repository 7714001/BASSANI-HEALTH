...
- [x] Generator origin toggle: **Bassani Produced (BH) / Imported Stock (BI)** — BH shows the existing four families unchanged; BI shows supplier + stock type + optional size, assembling `BI{sup}-{prod}{type}{ref}{sub}-{date}` live with zero typing (`build_import_batch_id`, unit-tested against 5 real register rows spanning all format variants)
...
- [x] **Quarantine + RP release:** every S6 receipt lands with `status: "quarantine"`; issue movements for the batch (and its derived stages) are blocked server-side (409) and greyed out on the Vault Logbook tiles ("Awaiting Responsible Pharmacist release", ledger returns `unreleased_imports`) until released
...
- [x] Manage Products modal (`BatchRegistry.js`): per-row link status ("Linked to X" green / "Not linked to a stock system product" amber) + **Link product** picker (search, pre-filled with the product name) + Unlink — mirrors the supplier UI in `S6Register.js`'s Manage Suppliers modal
