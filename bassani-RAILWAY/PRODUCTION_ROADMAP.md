...
- [x] `frontend/src/views/ManufacturingOrders.js` — row actions (Confirm / Record Production / Mark Complete), gated on `can("orders.manufacturing_manage")`. Record Production and Mark Complete render together whenever the MO is confirmed/progress/to_close (Record Production hidden once fully recorded, since there's nothing left to record); Record Production opens a `FormGroup`/`Input` modal pre-filled with the current recorded quantity, Mark Complete goes through a confirmation `Modal` (standing pattern — it's a real, hard-to-reverse Odoo stock movement), Confirm has no stock-side-effect and fires directly. On success: Confirm and Record Production patch the row locally (state, and for the latter `qty_producing`/`qty_remaining` too); Mark Complete removes the row (a `done` MO no longer matches this list's own open-MO domain, so it would vanish on the next refresh regardless)
