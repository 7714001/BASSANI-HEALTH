...
- [ ] **Not fixed:** the root cause of the false-positive status shown on a company's own Portal Access table (`get_portal_access`'s `users_map` matching a contact as already-active when no corresponding `companies[]` entry exists) — only worked around via the direct-add action above. Needs its own investigation; likely candidates are Odoo-side contact re-parenting after the original grant, or a second/legacy-shaped `users` document not caught by `GET /portal-logins/{email}` (e.g. different username casing/whitespace) — neither confirmed yet.
- [ ] Not built: an automatic fix at the source for the duplicate-login race (e.g. an atomic `find_one_and_update`/upsert in `grant_portal_access`, or a unique index on `username`) — this pass ships the detection + repair tool, not a guarantee the race can never happen again. Worth revisiting if it recurs.
