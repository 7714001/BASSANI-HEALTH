...
**Same-day, ninth round:**
- [x] Proof of Payment upload moved out of its own card's header into the Actions card (next to Reorder/Make Recurring), so every reseller/customer self-service action lives in one place; the card itself is now display-only, gated on `pop_uploads.length > 0` for every role instead of always rendering once a ticket exists. The timeline's own "Upload proof of payment" quick-access link on the current Deposit step is unchanged, kept deliberately for quick access
