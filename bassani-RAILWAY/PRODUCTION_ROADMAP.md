...
- [ ] Not built: an automatic fix at the source for the duplicate-login race (e.g. an atomic `find_one_and_update`/upsert in `grant_portal_access`, or a unique index on `username`) — this pass ships the detection + repair tools, not a guarantee a wrong-email grant or a same-email race can never happen again. Worth revisiting if either recurs.
