...
- [x] Sales Ticket detail modal — **Build Quote** button (when no `order_id`) opens product-search order builder: products loaded from `/api/products/`, filterable by name/SKU, qty stepper, warehouse selector, note; submits to `create-order` endpoint.
