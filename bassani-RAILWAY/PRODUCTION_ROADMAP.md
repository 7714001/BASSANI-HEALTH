...
- [x] **Frontend wiring (built 2026-07-27) — kept the bespoke `<table>` markup, did not switch to `DataTable`.** Investigating the swap surfaced a real conflict: `DataTable` (see `Invoices.js`) is light-mode only (zero `dark:` classes anywhere in that component or its reference usage) and has no expand/sub-row support, while Batch Registry's and Vault Ledger's tables are dark-mode-aware throughout and rely on the `Fragment`-based expand-to-fetch-detail row for their grouped view. Forcing the swap would have regressed dark mode on two production pages or required extending `DataTable` itself — out of scope for a pagination task. Instead, added one shared component, `Pager` (`components/UI.js`), a dark-mode-aware pagination footer with the same `pageIndex`/`pageSize`/`total` contract `DataTable` uses, and wired it into all four views (`BatchRegistry.js`, and `VaultLogbook.js`'s ledger table + movement history, `S6Register.js`) on top of their existing table markup. Each view resets its `pagination` state to page 1 (a new object reference, which reliably re-triggers the `load` `useCallback`/`useEffect` pair even when already on page 1) after any action that changes the underlying data, rather than calling `load()` directly — the latter would still close over the pre-update pagination value from the same render.
