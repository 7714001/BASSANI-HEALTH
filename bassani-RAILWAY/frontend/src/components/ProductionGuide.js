        ...
        <Modal title="Batch Numbers & Vault Guide" onClose={() => setOpen(false)} width="max-w-2xl">
          <div className="space-y-6 text-sm text-gray-600">
            ...
            <Section title="Reading a batch number">
              <div className="bg-gray-50 rounded-lg px-4 py-3 mb-2">
                <p className="font-mono text-lg font-semibold tracking-wide">
                  <span className="text-bassani-600">BH</span>
                  <span className="text-purple-600">DSD</span>
                  <span className="text-gray-400">-</span>
                  <span className="text-amber-600">011</span>
                  <span className="text-gray-400">-</span>
                  <span className="text-sky-600">100626</span>
                  <span className="text-gray-400">-</span>
                  <span className="text-rose-600">M</span>
                </p>
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-x-3 gap-y-1 mt-2 text-xs">
                  <span><span className="font-semibold text-bassani-600">BH</span> — Bassani Health</span>
                  <span><span className="font-semibold text-purple-600">DSD</span> — product shortcode (Dosi Si Dos)</span>
                  <span><span className="font-semibold text-amber-600">011</span> — batch number for that product</span>
                  <span><span className="font-semibold text-sky-600">100626</span> — date, day-month-year</span>
                  <span><span className="font-semibold text-rose-600">M</span> — current stage (Manicured)</span>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {FAMILY_ROWS.map(r => (
                      <tr key={r.label} className="align-top">
                        <td className="py-2 pr-3 font-medium text-gray-700 whitespace-nowrap">{r.label}</td>
            ...
            <Section title="Reading an imported batch number (BI)">
              <div className="bg-gray-50 rounded-lg px-4 py-3 mb-2">
                <p className="font-mono text-lg font-semibold tracking-wide">
                  <span className="text-bassani-600">BI</span>
                  <span className="text-teal-600">SB</span>
                  <span className="text-gray-400">-</span>
                  <span className="text-purple-600">JSY</span>
                  <span className="text-orange-600">3</span>
                  <span className="text-amber-600">40</span>
                  <span className="text-rose-600">L</span>
                  <span className="text-gray-400">-</span>
                  <span className="text-sky-600">300426</span>
                </p>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-3 gap-y-1 mt-2 text-xs">
                  <span><span className="font-semibold text-bassani-600">BI</span> — Bassani Import</span>
                  <span><span className="font-semibold text-teal-600">SB</span> — supplier (Seven Blade)</span>
                  <span><span className="font-semibold text-purple-600">JSY</span> — product shortcode (Jealousy)</span>
                  <span><span className="font-semibold text-orange-600">3</span> — stock type (Greenhouse)</span>
                  <span><span className="font-semibold text-amber-600">40</span> — the product's import number</span>
                  <span><span className="font-semibold text-rose-600">L</span> — size, flower only (Large / Pops / Smalls)</span>
                </div>
              </div>
              <p className="text-xs text-gray-500">
              ...
              <div className="space-y-1 text-xs">
                {STAGE_ROWS.map(r => (
                  <div key={r.suffix} className="flex gap-3">
                    <span className="font-mono font-semibold text-gray-700 w-24 shrink-0">-{r.suffix}</span>
                    <span>{r.label}</span>
                  </div>
                ))}
              </div>
              <div className="mt-3 bg-bassani-50 border border-bassani-100 rounded-lg px-3 py-2 text-xs">
                <span className="font-semibold text-bassani-700">The stage letter is replaced, not added to.</span>{" "}
            ...
            <Section title="Finished goods packaging codes">
              <div className="space-y-1 text-xs">
                {PACKAGING_ROWS.map(r => (
                  <div key={r.suffix} className="flex gap-3">
                    <span className="font-mono font-semibold text-gray-700 w-40 shrink-0">{r.suffix}</span>
                    <span>{r.label}</span>
                  </div>
                ))}
              </div>
            ...
            <Section title="Good to know">
              <ul className="list-disc pl-4 space-y-1 text-xs text-gray-500">
                ...
                <li>A <span className="font-medium text-amber-600">Staged</span> label means the entry is safely recorded and queued for the stock system. It is written through automatically once the production facility connection is live. Keep recording as normal.</li>
