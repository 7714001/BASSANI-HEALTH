...
const STEPS = [
  ...
  { label: "Sign Documents",   shortLabel: "Sign",    icon: PenLine      },
];
...
export default function PublicRegister() {
  ...
  const next = () => { if (validateStep()) { setErrors({}); setStep(s => s + 1); } };
          ...
          <div className="flex gap-1">
            {STEPS.map((_, i) => (
              <div key={i} className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${
                i < step ? "bg-bassani-400" : i === step ? "bg-bassani-600" : "bg-gray-200"
              }`} />
            ))}
          </div>
        </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={i} className="flex items-center flex-1 last:flex-none min-w-0">
                ...
                {i < STEPS.length - 1 && (
                  <div className={`flex-1 h-px mx-2 min-w-[8px] ${i < step ? "bg-bassani-300" : "bg-gray-200"}`} />
                )}
              </div>
            );
          })}
