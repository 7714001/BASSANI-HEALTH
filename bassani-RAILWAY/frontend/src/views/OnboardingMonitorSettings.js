// Settings tab for the onboarding pipeline's public display URL — a sibling
// of MonitorSettings.js (operations monitor), not a shared/generalised
// component. Worth generalising into a multi-monitor list once a third
// dashboard actually exists; not for one hypothetical now.
import { useState, useEffect } from "react";
import { ClipboardCheck, Copy, RefreshCw, CheckCircle } from "lucide-react";
import { BtnPrimary, BtnSecondary, Modal } from "../components/UI";
import api from "../api";
import toast from "react-hot-toast";

export default function OnboardingMonitorSettings({ embedded }) {
  ...
  const load = async () => {
    try {
      ...
      toast.error("Failed to load onboarding monitor token");
    } finally {
      setLoading(false);
    }
  };
  ...
  const monitorUrl = tokenData?.token
    ? `${window.location.origin}/onboarding-monitor?token=${tokenData.token}`
    : null;
  ...
  const doRotate = async () => {
    ...
    setRotating(true);
    try {
      await api.post("/api/onboarding-monitor/token");
      await load();
  ...
  const generateFirst = async () => {
    ...
    try {
      await api.post("/api/onboarding-monitor/token");
      await load();
      toast.success("Onboarding monitor URL generated.");
    } catch {
  ...
  const content = (
    <div className={embedded ? "p-6 max-w-4xl mx-auto w-full" : ""}>
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-base font-semibold text-gray-900">Onboarding Monitor</h2>
          <p className="text-sm text-gray-500">
            A live read-only board of the customer onboarding pipeline for TV / big-screen display. No login required — access is controlled by the URL token.
          </p>
        </div>
      </div>
      ...
      {rotateConfirm && (
        <Modal title="Rotate onboarding monitor token?" onClose={() => setRotateConfirm(false)}>
