...
function fmtDate(iso) {
  if (!iso) return "—";
  return new Date(iso).toLocaleDateString("en-ZA", { day: "2-digit", month: "short", year: "numeric", timeZone: "Africa/Johannesburg" });
}
...
function slotFileIcon(filename) {
  const ext = (filename || "").split(".").pop().toLowerCase();
  if (ext === "xlsx" || ext === "xls")
    return <Package size={14} className="text-green-600 shrink-0" />;
...
// ── Welcome Pack slot version history ─────────────────────────────────────────

function SlotVersionHistory({ slotName, isSuperAdmin, onActivated }) {
  const [versions,   setVersions  ] = useState([]);
  const [loading,    setLoading   ] = useState(true);
  const [activating, setActivating] = useState(null);

  useEffect(() => {
    api.get(`/api/doc-templates/welcome_pack/${slotName}/history`)
      .then(r => setVersions(r.data.versions || []))
      .catch(() => toast.error("Failed to load version history"))
      .finally(() => setLoading(false));
  }, [slotName]);
  ...
  const handleActivate = async (v) => {
    setActivating(v.id);
    try {
      await api.post(`/api/doc-templates/welcome_pack/${slotName}/activate/${v.id}`);
      toast.success(`v${v.version} is now active`);
      onActivated();
    } catch (e) {
      toast.error(e.response?.data?.detail || "Failed to activate version");
  ...
  const handleDownload = async (v) => {
    try {
      const res = await api.get(
        `/api/doc-templates/welcome_pack/${slotName}/download/${v.id}`,
        { responseType: "blob" }
      );
      const url = URL.createObjectURL(res.data);
      const a = document.createElement("a");
      a.href = url; a.download = v.filename || `${slotName}-v${v.version}`; a.click();
      URL.revokeObjectURL(url);
    } catch {
      toast.error("Download failed");
    }
  ...
  if (loading) return <p className="text-xs text-gray-400 py-2 pl-1">Loading…</p>;
  if (!versions.length) return <p className="text-xs text-gray-400 py-2 pl-1">No versions uploaded yet.</p>;
  ...
  return (
    <div className="mt-2 border border-gray-100 rounded-xl overflow-hidden">
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-gray-50 text-left">
            <th className="px-3 py-2 text-gray-500 font-medium">Version</th>
            <th className="px-3 py-2 text-gray-500 font-medium">File</th>
            <th className="px-3 py-2 text-gray-500 font-medium">Uploaded</th>
            <th className="px-3 py-2 text-gray-500 font-medium">By</th>
            <th className="px-3 py-2 text-gray-500 font-medium">Size</th>
            <th className="px-3 py-2 text-gray-500 font-medium">Notes</th>
        ...
        <tbody className="divide-y divide-gray-50">
          {versions.map(v => (
            <tr key={v.id} className={v.is_active ? "bg-green-50/40" : ""}>
              <td className="px-3 py-2"><VersionBadge version={v.version} isActive={v.is_active} /></td>
              <td className="px-3 py-2">
                <div className="flex items-center gap-1.5">
                  {slotFileIcon(v.filename)}
                  <span className="text-gray-600 truncate max-w-[140px]">{v.filename || "—"}</span>
                </div>
              </td>
              <td className="px-3 py-2 text-gray-600">{fmtDate(v.uploaded_at)}</td>
              <td className="px-3 py-2 text-gray-600 truncate max-w-[100px]">{v.uploaded_by_name || "—"}</td>
              <td className="px-3 py-2 text-gray-400">{fmtSize(v.file_size)}</td>
              <td className="px-3 py-2 text-gray-400 truncate max-w-[140px]">{v.notes || "—"}</td>
              <td className="px-3 py-2">
                <div className="flex items-center gap-2 justify-end">
                  <button onClick={() => handleDownload(v)} className="text-gray-400 hover:text-bassani-600 transition-colors" title="Download">
                    <Download size={13} />
...
// ── Individual Welcome Pack slot card ──────────────────────────────────────────

function SlotCard({ slot, isSuperAdmin, onUploaded }) {
  ...
  const acceptAttr = slot.accepts.join(",");

  const handleDownload = async () => {
    try {
      const res = await api.get(
        `/api/doc-templates/welcome_pack/${slot.slot}/download`,
        { responseType: "blob" }
      );
      const url = URL.createObjectURL(res.data);
      const a = document.createElement("a");
      a.href = url; a.download = active.filename; a.click();
      URL.revokeObjectURL(url);
    } catch {
      toast.error("Download failed");
    }
  ...
  const handleUpload = async () => {
    if (!chosenFile) return toast.error("Select a file first");
    setUploading(true);
    try {
      ...
      await api.post(
        `/api/doc-templates/welcome_pack/${slot.slot}/upload`,
        fd,
        { headers: { "Content-Type": "multipart/form-data" } }
      );
      toast.success(`${slot.label} updated to v${(active?.version || 0) + 1}`);
      setModalOpen(false);
      setHistoryKey(k => k + 1);
      onUploaded();
    } catch (e) {
  ...
  return (
    <>
      <div className="bg-gray-50 border border-gray-100 rounded-xl p-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center shrink-0">
              {slotFileIcon(active?.filename || slot.accepts[0])}
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium text-gray-800">{slot.label}</p>
              {active ? (
                <p className="text-xs text-gray-400 truncate">{active.filename}</p>
              ) : (
                <p className="text-xs text-amber-500">Not uploaded yet</p>
              )}
            </div>
          </div>
          {active && <VersionBadge version={active.version} isActive />}
        </div>
        ...
        {active?.notes && (
          <p className="text-xs text-gray-400 italic mb-3">Release note: {active.notes}</p>
        )}

        {/* Actions */}
        <div className="flex items-center gap-2 flex-wrap">
          {active && (
            <BtnSecondary size="sm" onClick={handleDownload}>
              <Download size={13} /> Download
            </BtnSecondary>
          )}
          {isSuperAdmin && (
            <BtnPrimary size="sm" onClick={() => { setChosenFile(null); setNotes(""); setModalOpen(true); }}>
              <Upload size={13} /> {active ? "Upload new version" : "Upload"}
            </BtnPrimary>
          )}
          <button
        ...
        >
          <div className="space-y-4">
            <p className="text-sm text-gray-500">
              The uploaded file will immediately become the active version for this document.
              The previous version is archived and can be restored at any time.
            </p>
            <FormGroup label="File" required>
              <div
              ...
              >
                {chosenFile ? (
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-400 mt-1">Accepted: {slot.accepts.join(", ")}</p>
                  </>
                )}
                <input
                  ref={fileRef}
            ...
            <FormGroup label="Release notes (optional)">
              <textarea
                ...
                placeholder="e.g. Updated pricing for Q3 2026"
                rows={3}
...
// ── Welcome Pack — 4-slot card ─────────────────────────────────────────────────

function WelcomePackSlotsCard({ template, isSuperAdmin, onUploaded }) {
  const [slots, setSlots] = useState(template.slots || []);

  const handleSlotUploaded = async () => {
    try {
      const r = await api.get("/api/doc-templates/welcome_pack/slots");
      setSlots(r.data.slots || []);
    } catch {}
    onUploaded();
  };

  const allUploaded = slots.every(s => !!s.active);
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
      <div className="flex items-start justify-between gap-4 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Welcome Pack</h3>
            <p className="text-xs text-gray-400 mt-0.5">4 documents — each independently versioned and maintained</p>
          </div>
        </div>
        {allUploaded ? (
          <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-green-50 text-green-700 shrink-0">
            <CheckCircle size={10} /> All uploaded
          </span>
        ) : (
          <span className="text-xs text-amber-600 font-medium shrink-0">
            {slots.filter(s => !s.active).length} missing
          </span>
        )}
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-gray-900/80 backdrop-blur-sm">
      ...
      <div className="flex-1 min-h-0 p-4">
        ...
        {objectUrl && (
          <iframe src={objectUrl} title={docLabel} className="w-full h-full rounded-xl border border-gray-200 bg-white" />
        )}
      </div>
    </div>
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const result = await generateSignedPdf(pdfBytes, {
        textValues, signingProfile: sigProfile, mikeFieldName,
        mikeImageBytes: sigBytes, customerSigDataUrl, config, addWatermark: true,
      });
      ...
      {loading ? (
        ...
        <div className="flex-1 min-h-0 flex">
          <div className="flex-1 p-4 min-w-0">
            {pdfUrl && <iframe src={pdfUrl} title={docLabel} className="w-full h-full rounded-xl border border-gray-200 bg-white" />}
          </div>
          <div className="w-80 shrink-0 border-l border-gray-200 bg-white flex flex-col">
                ...
                <div>
                  ...
                  {sigProfile ? (
                    ...
                    <div className="bg-amber-50 border border-amber-100 rounded-xl p-3">
                      <p className="text-xs text-amber-700">No signing identity configured. Set up your signing name and signature in My Profile first.</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="p-4 border-t border-gray-100 shrink-0">
              <BtnPrimary onClick={handleGenerate} disabled={generating} className="w-full justify-center">
                {generating ? <><Loader2 size={13} className="animate-spin" /> Generating…</> : <><Download size={13} /> Download signed test PDF</>}
              </BtnPrimary>
            </div>
          </div>
        </div>
...
function DocTypeCard({ template, isSuperAdmin, onUploaded }) {
  ...
  return (
    <>
      ...
      {viewOpen && (
        <PdfViewerModal docType={template.doc_type} docLabel={template.label} onClose={() => setViewOpen(false)} />
      )}
      {testOpen && (
        <TestSigningModal docType={template.doc_type} docLabel={template.label} onClose={() => setTestOpen(false)} />
      )}
      {modalOpen && (
    ...
    <div className={embedded ? "flex flex-col flex-1 overflow-hidden" : "flex flex-col min-h-screen bg-gray-50"}>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="max-w-4xl mx-auto w-full">
          ...
          <div className="mb-6">
            <p className="text-sm text-gray-500">
              Manage the four Bassani-issued template documents. The three onboarding PDFs are
              single-file with full version history. The Welcome Pack is managed as four
              independently versioned documents — update any one without touching the others.
            </p>
            {isSuperAdmin && (
              <div className="mt-3 flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
                ...
                <div className="flex-1">
                  <p className="text-xs text-blue-700">
                    ...
                    Each field must use the exact name the portal expects.
                  </p>
                  <button
          ...
          <div className="mt-8 bg-gray-100 rounded-2xl px-6 py-5">
            ...
            <ul className="space-y-1.5 text-xs text-gray-500">
              ...
              <li>The active version is served immediately — no redeployment required.</li>
              <li>Archived versions are never deleted and can be restored at any time.</li>
              <li>Welcome Pack documents are managed individually — updating the price list does not affect the brochure or any other slot.</li>
              <li>When a welcome pack email is sent, the active version of each slot is attached automatically.</li>
            </ul>
          </div>
        </div>
      </main>
              ...
              {FIELD_REF_DOCS.map(d => (
                <button
                  ...
                  className={`px-3 py-2 text-xs font-medium border-b-2 -mb-px transition-colors whitespace-nowrap ${
                    fieldRefTab === d.key ? "border-bassani-600 text-bassani-700" : "border-transparent text-gray-500 hover:text-gray-700"
                  }`}
                >
                  {d.label}
                </button>
              ...
              <div key={d.key} className="space-y-4">
                ...
                {cfg?.hasBassaniSig && (
                  <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 text-xs text-amber-700 space-y-2">
                    <p>This document requires a Bassani countersignature. Add a signature field with a name starting with <span className="font-mono font-semibold">bassani_</span>.</p>
                    {cfg.bassaniTextFields?.length > 0 && (
                      <>
                        <p className="font-semibold pt-1">These text fields are auto-filled by the portal:</p>
                        <div className="border border-amber-200 rounded-lg overflow-hidden">
                          <table className="w-full">
                            <thead><tr className="bg-amber-100/60 border-b border-amber-200"><th className="text-left px-3 py-1.5 font-semibold w-1/2">Field name</th><th className="text-left px-3 py-1.5 font-semibold">Filled with</th></tr></thead>
                            <tbody>
