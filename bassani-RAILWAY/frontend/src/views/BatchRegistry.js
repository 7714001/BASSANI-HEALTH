...
export const SYNC_BADGE = {
  staged: { label: "Staged",  cls: "bg-amber-100 text-amber-700" },
  done:   { label: "In Odoo", cls: "bg-green-100 text-green-700" },
  error:  { label: "Sync error", cls: "bg-red-100 text-red-700" },
};
                  ...
                  {(meta?.families || []).map(f => (
                    <button
                      ...
                      className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-colors ${
                        family === f.key
                          ? "bg-bassani-100 text-bassani-700 ring-1 ring-bassani-300"
                          : "bg-gray-50 text-gray-500 hover:text-gray-700 border border-gray-200"
                      }`}
                    >
                      {f.label}
                    </button>
              ...
              <div className="flex-1 bg-gray-50 border border-dashed border-gray-300 rounded-lg px-4 py-3">
                {preview ? (
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <button onClick={() => copyId(preview.batch_id)} className="text-gray-400 hover:text-gray-600" title="Copy">
                      <Copy size={14} />
                    </button>
                  </div>
                ) : (
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 gap-3">
              ...
              <input
                ...
                className="text-xs border border-gray-200 rounded-lg px-3 py-1.5 bg-gray-50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-bassani-400 w-56"
              />
            </div>
                      ...
                      return (
                        <Fragment key={g.baseId}>
                          ...
                          >
                            ...
                            <td className="px-5 py-3 text-gray-600 max-w-[180px] truncate">{g.product_name}</td>
                            <td className="px-5 py-3 text-xs whitespace-nowrap">
                              <div className="flex flex-wrap items-center gap-1">
                                {g.leaves.map(l => (
                                  <span key={l.batch_id} className="text-[11px] font-medium px-1.5 py-0.5 rounded bg-gray-100 text-gray-600">
                                    {l.stage_suffix ? (STAGE_LABELS[l.stage_suffix] || l.stage_suffix) : "Base"}
                                  </span>
                                ))}
                                {g.stageCount > g.leaves.length && (
                                  <span className="text-[10px] text-gray-400">({g.stageCount} stages)</span>
                                )}
                              </div>
                            </td>
                            <td className="px-5 py-3 text-xs text-gray-400 whitespace-nowrap">
                              {fmtWhen(g.started_at)}
                              <span className="block text-gray-400">{g.started_by}</span>
                            </td>
                          ...
                          {isOpen && (
                            <tr>
                              <td colSpan={5} className="px-5 py-4 bg-gray-50/70">
                                {!detail || detail.loading ? (
                                  ...
                                  <div className="space-y-4">
                                    {/* Stage chain */}
                                    <div>
                                      <p className="text-xs font-medium text-gray-500 mb-1.5">Stages of this batch</p>
                                      <div className="flex flex-wrap gap-1.5">
                                        {detail.stages.map(s => (
                                          <span key={s.batch_id} title={batchTitle(s.batch_id, s.product_name)} className="inline-flex items-center gap-1.5 font-mono text-xs px-2 py-1 rounded-lg border border-gray-200 bg-white text-gray-600">
                                            {s.batch_id}
                                    ...
                                    {/* Movement history */}
                                    <div>
                                      <p className="text-xs font-medium text-gray-500 mb-1.5">Vault movements</p>
                                      {detail.movements.length === 0 ? (
                                        <p className="text-sm text-gray-400">No vault movements recorded for this batch yet.</p>
                                      ) : (
                                        <div className="space-y-1.5">
                                          {detail.movements.map(m => (
                                            <div key={m.id} className="flex items-start justify-between gap-3 text-sm bg-white border border-gray-100 rounded-lg px-3 py-2">
                                              <div className="min-w-0">
                                                <p className="text-gray-700 text-xs font-medium">{MOVE_LABEL[m.type] || m.type}</p>
        ...
        <Modal title="Manage Products" onClose={() => setManageOpen(false)} width="max-w-2xl">
          ...
          <div className="flex flex-col sm:flex-row gap-2 mb-4">
            ...
            <input
              ...
              maxLength={4}
              placeholder="Code"
              className="w-full sm:w-24 text-sm font-mono border border-gray-200 rounded-lg px-3 py-2 bg-gray-50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-bassani-400"
            />
          ...
          <input
            ...
            className="w-full text-xs border border-gray-200 rounded-lg px-3 py-1.5 bg-gray-50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-bassani-400 mb-2"
          />
            ...
            {manageList
              ...
              .map(s => (
                <div key={s.code} className={`flex items-center gap-3 px-3 py-2 text-sm ${s.active ? "" : "opacity-60"}`}>
                  <span className="font-mono text-xs font-semibold text-gray-500 w-12 shrink-0">{s.code}</span>
                  <div className="flex-1 min-w-0">
                    ...
                    {s.odoo_product_name ? (
                      ...
                      <span className="block text-xs text-amber-600">Not linked to a stock system product</span>
                    )}
                  </div>
                  {!s.active && (
        ...
        <Modal title={`Link ${linkTarget.name}`} onClose={() => setLinkTarget(null)} width="max-w-2xl">
          ...
          <div className="relative mb-2.5">
            ...
            <input
              ...
              autoFocus
              className="w-full text-sm pl-9 pr-3 py-2.5 border border-gray-200 rounded-xl bg-gray-50 text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-bassani-400"
            />
          </div>
          ...
          <div className="max-h-72 overflow-y-auto divide-y divide-gray-100 border border-gray-100 rounded-lg">
            {odooProductSearching ? (
              ...
              return (
                ...
                >
                  <div className="min-w-0">
                    <span className="block text-gray-800 font-medium truncate">{base}</span>
                  ...
                  <span className="text-xs font-semibold text-bassani-600 shrink-0">Link</span>
                </button>
              );
            })}
          </div>
