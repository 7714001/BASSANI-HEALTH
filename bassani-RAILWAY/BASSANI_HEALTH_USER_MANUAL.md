...
**Category Mapping (bulk setup):** Building up a full set of category mappings one Parent Category at a time can be slow when there are dozens of Odoo categories to place. Go to **Category Mapping** for a faster way: one row per real Odoo category, with a Parent Category dropdown and a Sub Category dropdown next to it. Picking a Parent Category from the dropdown assigns that Odoo category to it immediately — no save button, no modal. If the Parent Category you picked has sub-categories of its own, the Sub Category dropdown fills in so you can assign it more precisely (e.g. under "Vapes", to either "CannaCrafter's Vapes" or "Green Clouds Vapes" specifically); leaving Sub Category on "None" keeps it directly under the parent. Re-picking a different Parent Category for a row simply moves it — a category only ever has one home. Use the search box and the "Unmapped only" toggle to work through a long category list without losing your place, and check the subtitle at the top of the page for a running count of how many categories are mapped. You still create the Parent Categories and sub-categories themselves (name, hand-picked products, etc.) on the **Parent Categories** page first — this page is only for assigning Odoo categories to categories that already exist.
