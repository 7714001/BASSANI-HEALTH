...
**Sub-categories (brand/grade drill-down):** A Parent Category can optionally sit inside another one, one level deep, using the "Parent category" field on its edit form. This is for cases where a top-level family is really made up of several real Odoo groupings underneath — for example, Bassani's "Flower" isn't one Odoo category, it's twenty (four grades — Indoor, Exotic, Greendoor, Greenhouse — each split into five formats). Create "Flower" first as a top-level category with no Odoo categories of its own, then create "Indoor", "Exotic", "Greendoor", and "Greenhouse" each with "Flower" set as their parent and their own five real Odoo categories selected. The same works for a family with real brands underneath, like "Vapes" containing "CannaCrafter's Vapes" and "Green Clouds Vapes". Once set up, a reseller who selects "Flower" immediately sees every product across all four grades, and can then narrow to just "Indoor" using the Brand/Grade dropdown. A category that already has its own children can't be nested under another one, and only top-level categories can be picked as a parent — both are enforced so nesting never goes more than two levels deep.
