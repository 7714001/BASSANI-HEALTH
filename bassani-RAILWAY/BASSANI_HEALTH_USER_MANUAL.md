...
**Variant Aliases (friendly variant names):** Products in Odoo often carry cryptic attribute codes as part of their variant — "GD," "GH," "EXO," "IND" — which mean nothing to a reseller. The third tab on the **Parent Categories** page, **Variant Aliases**, lets you map each code to a friendly name once: enter the Odoo code (e.g. "GD") and the name you want resellers to see instead (e.g. "Greendoor"), click **Add**, then **Save Changes**. From then on, that code is translated everywhere a reseller sees a variant — the Product Catalog's variant chips and dropdown, and the order cart's product cards and Variant dropdown. Matching is not case-sensitive. Staff-facing screens (the internal Products page, the quote builder) are unaffected and keep showing Odoo's raw codes, since staff already work with them daily.
