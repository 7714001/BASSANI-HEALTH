...
**A customer with their own portal login doesn't have to rely on the email.** The moment a repeat order is generated, it also shows up as a banner on their Dashboard and as a review card right on the order's own page — Accept and Decline work exactly the same way there as they do on the emailed link. This is customer logins only for now; a reseller managing a customer's recurring schedule still relies on the email being forwarded, or checking the ticket queue.

On **Recurring Orders** (`/orders/recurring`), you can also click the lightning-bolt **Run Now** icon next to any active schedule to generate its next occurrence immediately instead of waiting for the automatic 2-days-before timing. This is a real action — it creates the actual draft order and sends the actual customer email — so use it deliberately, not for casual testing on a live customer's schedule.
...
**Light/dark theme:** same toggle and fullscreen button as the Operations Monitor (2026-08-23), dark by default. This board is company/application-focused rather than order-focused, so cards are not grouped the way the Operations and Manufacturing monitors are.
...
**Reconciling cancelled tickets (admin only)**
A **Reconcile Cancelled** button appears in the top bar for anyone with Admin Override permission. Click it if an order ticket is still showing active (and still appearing on the Operations Monitor) even though its Sales ticket was already cancelled or marked Not Interested. Clicking it never cancels anything by itself — it first shows you a preview listing exactly which order tickets would be cancelled and why (their reference, customer, current status, and the Sales ticket's exit status). Nothing is changed until you review that list and click Confirm inside it; if there is nothing to fix, it simply tells you so and closes. This is safe to run at any time — clicking it again once everything is already reconciled just shows an empty result. You will only normally need this after an ERP-side order cancellation that happened before the automatic sync existed, or if you are ever told by support that a specific batch of records needs reconciling.
...
As you build a Parent Category, a **Preview** panel updates automatically below the two pickers, showing exactly which products currently match your selection before you save anything. Each product row has a **Visible / Hidden** toggle switch you can flip right there — no need to leave the modal and go find the product on the Products page. A blue note under a hand-picked product that isn't visible yet lets you know it will be made visible automatically when you save (or you can just flip it on now). If the category has sub-categories of its own, the preview only reflects what's added directly to it — open each sub-category to preview its own matches.

**Bulk Reseller Catalog / MOQ update:** To change which products are reseller-visible, and their minimum order quantities, for many products at once instead of one at a time, use the **Import Reseller Catalog / MOQ** button next to Export for Odoo. First click **Export for Odoo** to download the current product sheet — it includes a **Reseller Catalog** column (Yes/No) and an **MOQ** column alongside the category-mapping ones. Edit those two columns in Excel for whichever products need to change, then click **Import Reseller Catalog / MOQ** and upload the edited file. Nothing is changed yet at this point — you'll see a preview listing exactly which products will be added to or removed from the reseller catalog, and which MOQs will change, each with its own checkbox (all pre-checked). Untick anything you don't want applied, then click **Apply**. Setting an MOQ on a row marked "No" for Reseller Catalog has no effect — the MOQ only matters once a product is actually visible, so it's ignored and flagged in the preview rather than silently saved. Only click Apply once you're satisfied with the preview; this writes to the same reseller catalog every toggle on the Products page uses, so undoing a mistaken bulk apply means re-running the import with corrected values or fixing individual products by hand.
...
A **Products** column on this table shows how many products are actually sitting in each Odoo category right now, so you can tell at a glance which unmapped ones matter. A category showing **0** is safe to leave unmapped — there's nothing in it. Click any non-zero number to see exactly which products are in that category. Click the column header to sort by it, so the categories with the most unmapped products rise to the top. If any unmapped category has products in it, an amber banner appears above the table with a **Show these** shortcut that filters and sorts straight to them.
...
**By Order view (default):** One row per backorder picking. Each row shows an **Age** column (days since the backorder was created, colour-coded amber past 1 day and red past 3) and an **Outstanding** column with the total units outstanding and how many products are affected. Click the chevron to expand the row and see each product on its own line underneath, with its individual outstanding quantity and any linked Manufacturing Order — only the first row on the page starts expanded, the rest start collapsed so the page reads as a summary at a glance (2026-08-23). Click the **sale order reference** (e.g. S00042) in the first column to open the Order Passport for that order — the full lifecycle view showing ticket stage, invoice, deliveries, and batch numbers. Each row also links to the portal Sales Ticket if one exists.
...
**Checking or fixing which companies a login has access to:** click **Manage** next to any Active or Deactivated row in the Portal Access table (or, from the **Users** screen, the building-icon button next to a customer-role account's Reset Password button — both open the same view). This shows every company that login currently switches between, with an activate/deactivate toggle per company. If access was granted on two different company profiles for the same person around the same time, it's possible to end up with two separate logins instead of one — the customer would then only ever see one company in their switcher no matter how many times access looks "granted" on each company's own page. The Manage view shows this immediately as an amber warning listing both logins; pick which one to keep (it defaults to whichever one has actually been used to sign in, so the customer's existing password keeps working) and click **Merge Into Selected**, then confirm. This moves every company from the other login onto the one you kept and removes the duplicate — the customer's switcher will show all their companies the next time they load the app.

**If a contact's row shows Active/Deactivated but they should have this company too and there's no checkbox to grant it:** opening **Manage** from that company's own profile page shows a blue "This login doesn't have [company] yet" banner with an **Add This Company** button — this grants access to this specific company directly, without needing the row's own checkbox. If a duplicate-login warning also appears, merge the duplicates first — the button stays disabled until you do, so a new company is never added to the wrong one of two logins.

**If Add This Company doesn't work, or a row's email looks right but you can't tell whose login it actually is:** a contact can end up granted under the *wrong* email entirely — for example if someone typed an internal address by mistake instead of the customer's real one while setting up access. When this happens, the Portal Access table shows a red note under the contact's email: "Wrong account: this status belongs to [other email], not this contact's own email." Deactivating that row won't fix it, since the wrong login still holds the claim on that contact and blocks it from ever being granted correctly. Instead, click the **Remove (wrong account)** button that appears on that row, confirm, and the contact becomes available to grant again — this time tick its checkbox as normal and it'll go to the right login.
...
- **Set an order to repeat automatically.** Tick "Make this a recurring order" in the cart before placing or saving the order to set it up in the same step, or use the "Make Recurring" button on any existing order's page afterwards — both set it up to repeat weekly, every two weeks, or monthly, the same feature a Sales Agent already has. Two days before each repeat, a card appears right on that order's page (and a banner on the Dashboard) asking the customer to accept or decline — the same review they'd get by email, just also reachable without leaving the portal. If they don't respond either way, that occurrence is simply skipped and the schedule continues from the next date on its own.
...
If an order is cancelled directly in the ERP (Odoo), the portal detects this the next time anyone opens that ticket and closes it automatically. Odoo is the financial source of truth — if the order is gone there, the ticket reflects that immediately. An amber notice on the closed ticket explains that it was auto-closed due to an ERP cancellation. Check the ticket's timeline for the "Auto-closed: Odoo order was cancelled" entry, and investigate the cancellation in Odoo if it was unexpected. If a linked Order Ticket (packing board card) had already been created for this order, it is automatically cancelled at the same time and disappears from the Operations Monitor within a few seconds — you never need to separately cancel it on the Orders Tickets page. This only applies going forward, from the moment the sync was fixed (2026-09-07) — it does not run again for a ticket that was already closed before then. If you find an older, already-cancelled ticket whose Order Ticket is still active, reopening the Sales ticket will not fix it (the sync only fires once, at the moment a ticket actually closes); use the **Reconcile Cancelled** button on the Orders Tickets page instead (see the Orders Clerk section above) — it previews every such entry across the whole board before you confirm, and is safe to run at any time.
