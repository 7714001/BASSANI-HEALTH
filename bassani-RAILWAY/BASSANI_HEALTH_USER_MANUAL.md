...
**If a contact's row shows Active/Deactivated but they should have this company too and there's no checkbox to grant it:** opening **Manage** from that company's own profile page shows a blue "This login doesn't have [company] yet" banner with an **Add This Company** button — this grants access to this specific company directly, without needing the row's own checkbox (which occasionally shows the wrong status). If a duplicate-login warning also appears, merge the duplicates first — the button stays disabled until you do, so a new company is never added to the wrong one of two logins.
